...
pub trait ArrayTraitExt<T> {
    ...
    fn pop_front_n<+Destruct<T>>(ref self: Array<T>, n: usize) -> Array<T>;
    /// Removes up to `n` elements from the front of `self`.
    fn remove_front_n<+Destruct<T>>(ref self: Array<T>, n: usize);
...
impl ArrayImpl<T, +Clone<T>, +Drop<T>> of ArrayTraitExt<T> {
    ...
    fn pop_front_n<+Destruct<T>>(ref self: Array<T>, mut n: usize) -> Array<T> {
        let mut res = array![];
    ...
    fn remove_front_n<+Destruct<T>>(ref self: Array<T>, mut n: usize) {
        while (n != 0) {
            match self.pop_front() {
                Option::Some(_) => { n -= 1; },
                Option::None => { break; },
