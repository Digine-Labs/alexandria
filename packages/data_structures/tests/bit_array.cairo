...
fn test_append_bit() {
    ...
    assert(ba.data() == expected, 'values should match');
}
...
fn test_at() {
    ...
    loop {
        if index == 8 * 16 - 1 {
            // last value
            assert(ba[index] == false, 'values should match');
            break;
        }
        assert(ba.at(index).unwrap() == true, 'values should match');
        index += 1;
    };
}
...
fn test_at_none() {
    let ba = sample_bit_array();
    assert(ba.at(8 * 16).is_none(), 'should be none');
}

#[test]
fn test_index() {
    let ba = sample_bit_array();
    assert(ba[0] == true, 'values should match');
    assert(ba[8 * 16 - 1] == false, 'values should match');
}
...
fn test_len() {
    ...
    let expected = 8 * 16;
    assert(ba.len() == expected, 'values should match');
    ba.append_bit(true);
    assert(ba.len() == expected + 1, 'values should match');
    let _ = ba.pop_front();
    assert(ba.len() == expected, 'values should match');
}
...
fn test_pop_front() {
    let mut ba = sample_bit_array();
    assert(ba.pop_front() == Option::Some(true), 'values should match');
}

#[test]
fn test_pop_front_empty() {
    let mut ba: BitArray = Default::default();
    assert(ba.pop_front() == Option::None, 'values should match');
}
...
fn test_read_word_be_u256() {
    ...
    assert(ba.read_word_be_u256(length: 256).unwrap() == u256 { low, high }, 'u256 be mismatch');
}
...
fn test_read_word_le_u256() {
    ...
    assert(ba.read_word_le_u256(length: 256).unwrap() == u256 { low, high }, 'u256 le mismatch');
}
...
fn test_read_word_be_half() {
    let mut ba = sample_bit_array();
    assert(ba.read_word_be(64).unwrap() == 0xffffffffffffffff, 'values should match');
    assert(ba.read_word_be(64).unwrap() == 0xfffffffffffffffe, 'values should match');
}

#[test]
fn test_read_word_le_half() {
    let mut ba = sample_bit_array();
    assert(ba.read_word_le(64).unwrap() == 0xffffffffffffffff, 'values should match');
    assert(ba.read_word_le(64).unwrap() == 0x7fffffffffffffff, 'values should match');
}
...
fn test_write_word_be() {
    ...
    assert(ba.read_word_be(128).unwrap() == Bounded::<u128>::MAX.into() - 2, 'values should match');
}
...
fn test_write_word_be_half() {
    ...
    assert(ba.read_word_be(64).unwrap() == Bounded::<u64>::MAX.into() - 3, 'values should match');
}
...
fn test_write_word_le() {
    ...
    assert(ba.read_word_le(128).unwrap() == Bounded::<u128>::MAX.into() - 4, 'values should match');
}
...
fn test_write_word_le_half() {
    ...
    assert(ba.read_word_le(64).unwrap() == Bounded::<u64>::MAX.into() - 5, 'values should match');
}
...
fn test_write_word_be_u256() {
    ...
    assert(ba.read_word_be_u256(256).unwrap() == expected, 'values should match');
}
...
fn test_write_word_le_u256() {
    ...
    assert(ba.read_word_le_u256(256).unwrap() == expected, 'values should match');
}
...
fn test_write_word_be_u512() {
    ...
    assert(ba.read_word_be_u512(512).unwrap() == expected, 'values should match');
}
...
fn test_write_word_le_u512() {
    ...
    assert(ba.read_word_le_u512(512).unwrap() == expected, 'values should match');
}
...
fn test_stress_test() {
    ...
    while (index != limit) {
        ...
        assert(value == index.into(), 'values should match');
...
fn test_serde_serialize() {
    ...
    assert(length == ba.len(), 'values should match');
    ...
    assert(data == expected, 'values should match');
}
...
fn test_serde_ser_deser() {
    ...
    assert(deser.len() == ba.len(), 'values should match');
    assert(deser.read_word_be(248).unwrap() == test, 'values should match');
    assert(deser.read_word_be(248).unwrap() == test + 1, 'values should match');
    assert(deser.read_word_be(248).unwrap() == test + 2, 'values should match');
    assert(deser.read_word_be(248).unwrap() == test + 3, 'values should match');
    assert(deser.read_word_be(248).unwrap() == test + 4, 'values should match');
    assert(deser.read_word_be(248).unwrap() == test + 5, 'values should match');
}
