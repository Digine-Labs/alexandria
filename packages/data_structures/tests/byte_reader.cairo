...
fn test_word_u16() {
    ...
    assert(word == 0x3f40_u16, 'values should match');
}
...
fn test_word_u16_le() {
    ...
    assert(word == 0x403f_u16, 'values should match');
}
...
fn test_word_u32() {
    ...
    assert(word == 0x3d3e3f40_u32, 'values should match');
}
...
fn test_word_u32_le() {
    ...
    assert(word == 0x403f3e3d_u32, 'values should match');
}
...
fn test_word_u64() {
    ...
    assert(word == 0x393a3b3c3d3e3f40_u64, 'values should match');
}
...
fn test_word_u64_le() {
    ...
    assert(word == 0x403f3e3d3c3b3a39_u64, 'values should match');
}
...
fn test_word_u128() {
    ...
    assert(word == 0x3132333435363738393a3b3c3d3e3f40_u128, 'values should match');
}
...
fn test_word_u128_le() {
    ...
    assert(word == 0x403f3e3d3c3b3a393837363534333231_u128, 'values should match');
}
...
fn test_reader_helper() {
    ...
    // Test that reader can successfully read from the data
    assert(reader.read_u8().is_some(), 'reader should work');
    assert(reader.len() == ba.len() - 1, 'length should decrease');
}
...
fn test_len() {
    ...
    assert(64 == rd.len(), 'values should match');
    let _ = rd.read_u8().expect('some');
    assert(63 == rd.len(), 'values should match');
    let _ = rd.read_u256().expect('some');
    assert(31 == rd.len(), 'values should match');
    let _ = rd.read_u128().expect('some');
    assert(15 == rd.len(), 'values should match');
    let _ = rd.read_u64().expect('some');
    assert(7 == rd.len(), 'values should match');
    let _ = rd.read_u32().expect('some');
    assert(3 == rd.len(), 'values should match');
    let _ = rd.read_u16().expect('some');
    assert(1 == rd.len(), 'values should match');
    let _ = rd.read_i8().expect('some');
    assert(0 == rd.len(), 'values should match');
}
...
fn test_read_u256() {
    ...
    assert(high1 == 0x0102030405060708090a0b0c0d0e0f10_u128, 'values should match');
    assert(low1 == 0x1112131415161718191a1b1c1d1e1f20_u128, 'values should match');
}
...
fn test_read_u256_le() {
    ...
    assert(high1 == 0x201f1e1d1c1b1a191817161514131211_u128, 'values should match');
    assert(low1 == 0x100f0e0d0c0b0a090807060504030201_u128, 'values should match');
}
...
fn test_read_u512() {
    ...
    assert(limb3 == 0x0102030405060708090a0b0c0d0e0f10_u128, 'values should match');
    assert(limb2 == 0x1112131415161718191a1b1c1d1e1f20_u128, 'values should match');
    assert(limb1 == 0x2122232425262728292a2b2c2d2e2f30_u128, 'values should match');
    assert(limb0 == 0x3132333435363738393a3b3c3d3e3f40_u128, 'values should match');
}
...
fn test_read_u512_le() {
    ...
    assert(limb0 == 0x100f0e0d0c0b0a090807060504030201_u128, 'values should match');
    assert(limb1 == 0x201f1e1d1c1b1a191817161514131211_u128, 'values should match');
    assert(limb2 == 0x302f2e2d2c2b2a292827262524232221_u128, 'values should match');
    assert(limb3 == 0x403f3e3d3c3b3a393837363534333231_u128, 'values should match');
}
...
fn test_read_sequence() {
    let ba = test_byte_array_64();
    let mut rd = ba.reader();
    assert(rd.read_i8() == Option::Some(1), 'values should match');
    assert(
    ...
    assert(rd.read_i64() == Option::Some(0x2223242526272829), 'values should match');
    assert(
        rd.read_u128() == Option::Some(0x2a2b2c2d2e2f30313233343536373839), 'values should match',
    );
    assert(rd.read_u32() == Option::Some(0x3a3b3c3d), 'values should match');
    assert(rd.read_i16() == Option::Some(0x3e3f), 'values should match');
    assert(rd.read_u8() == Option::Some(0x40), 'values should match');
    assert(rd.read_u8().is_none(), 'should be none');
}
...
fn test_read_sequence_le() {
    ...
    assert(rd.read_i64_le() == Option::Some(0x2928272625242322), 'values should match');
    assert(
        rd.read_u128_le() == Option::Some(0x393837363534333231302f2e2d2c2b2a),
        'values should match',
    );
    assert(rd.read_u32_le() == Option::Some(0x3d3c3b3a), 'values should match');
    assert(rd.read_i16_le() == Option::Some(0x3f3e), 'values should match');
    assert(rd.read_u8() == Option::Some(0x40), 'values should match');
    assert(rd.read_u8().is_none(), 'should be none');
}
...
fn test_clone_byte_array_reader() {
    ...
    assert(rd1.len() == rd2.len(), 'values should match');
    assert(a == b, 'values should match');
}
...
fn test_byte_array_reader_equals_array_of_bytes_reader() {
    ...
    assert(ba.read_u16() == bb.read_u16(), 'values should match');
    assert(ba.read_u16_le() == bb.read_u16_le(), 'values should match');
    assert(ba.read_i16() == bb.read_i16(), 'values should match');
    assert(ba.read_i16_le() == bb.read_i16_le(), 'values should match');
    assert(ba.read_u256() == bb.read_u256(), 'values should match');
}
