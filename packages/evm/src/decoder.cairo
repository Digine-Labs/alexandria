...
fn decode_bytes_32(ref ctx: EVMCalldata) -> Span<felt252> {
    ...
    ctx.offset = new_offset;
    let msb_byte = U256BitShift::shr(value, 248); // Shift 31 bytes (248 bits)
    let remaining_bytes = value
        & 0x00FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;

    array![
        remaining_bytes.try_into().unwrap(), // Last 31 bytes (big-endian)
        msb_byte.try_into().unwrap() // First byte (MSB)
    ]
        .span()
}
