...
fn interp_constant_left_diff() {
    ...
    assert(@interpolate(0, xs, ys, inter, extra) == @0, 'assertion failed');
    assert(@interpolate(1, xs, ys, inter, extra) == @2, 'assertion failed');
    assert(@interpolate(2, xs, ys, inter, extra) == @2, 'assertion failed');
    assert(@interpolate(3, xs, ys, inter, extra) == @4, 'assertion failed');
    assert(@interpolate(4, xs, ys, inter, extra) == @4, 'assertion failed');
    assert(@interpolate(5, xs, ys, inter, extra) == @6, 'assertion failed');
    assert(@interpolate(6, xs, ys, inter, extra) == @6, 'assertion failed');
    assert(@interpolate(7, xs, ys, inter, extra) == @8, 'assertion failed');
    assert(@interpolate(8, xs, ys, inter, extra) == @8, 'assertion failed');
    assert(@interpolate(9, xs, ys, inter, extra) == @8, 'assertion failed');
}
