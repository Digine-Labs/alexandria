...
fn test_deploy() {
    ...
    assert(contract.do_get_len() == (0, 0), 'assertion failed');
}
...
fn test_new_initializes_empty_list() {
    ...
    // assert(addresses_list.address_domain == 0, 'assertion failed'); // Private member access
    assert(addresses_list.len() == 0, 'assertion failed');
    // assert(addresses_list.base.into() == addresses_address, 'assertion failed'); // Private
    // member access
    assert(addresses_list.storage_size() == 1, 'assertion failed');
    ...
    // assert(numbers_list.address_domain == 0, 'assertion failed'); // Private member access
    assert(numbers_list.len() == 0, 'assertion failed');
    // assert(numbers_list.base.into() == numbers_address, 'assertion failed'); // Private member
    // access
    assert(numbers_list.storage_size() == 2, 'assertion failed');

    // Check if both addresses and numbers lists are initialized to be empty
    assert(contract_state.do_get_len() == (0, 0), 'assertion failed');
    assert(contract_state.do_is_empty() == (true, true), 'assertion failed');
}
...
fn test_new_then_fill_list() {
    ...
    assert(addresses_list.len() == 1, 'assertion failed');
    assert(numbers_list.len() == 2, 'assertion failed');

    assert(contract_state.do_get_len() == (1, 2), 'assertion failed');
    assert(contract_state.do_is_empty() == (false, false), 'assertion failed');
}
...
fn test_fetch_empty_list() {
    ...
    // assert(empty_list.address_domain == 0, 'assertion failed'); // Private member access
    assert(empty_list.len() == 0, 'assertion failed');
    // assert(empty_list.base.into() == storage_address, 'assertion failed'); // Private member
    // access
    assert(empty_list.storage_size() == 1, 'assertion failed');
}
...
fn test_fetch_existing_list() {
    ...
    assert(contract_state.do_append(mock_addr, 10) == (0, 0), 'assertion failed');
    assert(contract_state.do_append(mock_addr, 20) == (1, 1), 'assertion failed');

    let addresses_address = contract_state.addresses.as_ptr().__storage_pointer_address__;
    let addresses_list = ListTrait::<ContractAddress>::fetch(0, addresses_address)
        .expect('List fetch failed');
    // assert(addresses_list.address_domain == 0, 'assertion failed'); // Private member access
    assert(addresses_list.len() == 2, 'assertion failed');
    // assert(addresses_list.base.into() == addresses_address, 'assertion failed'); // Private
    // member access
    assert(addresses_list.storage_size() == 1, 'assertion failed');
...
fn test_is_empty() {
    ...
    assert(contract.do_is_empty() == (true, true), 'assertion failed');
    contract.do_append(mock_addr(), 1337);
    assert(contract.do_is_empty() == (false, false), 'assertion failed');
}
...
fn test_append_few() {
    ...
    assert(contract.do_append(mock_addr(), 10) == (0, 0), 'assertion failed');
    assert(contract.do_append(mock_addr(), 20) == (1, 1), 'assertion failed');
    assert(contract.do_get_len() == (2, 2), 'assertion failed');
}
...
fn test_append_get_many() {
    ...
    while (index != max) {
        ...
        assert(append_indexes == (index, index), 'assertion failed');
        index += 1;
    }

    assert(contract.do_get_len() == (max, max), 'assertion failed');
    ...
    while (index != max) {
        ...
        assert(some_addr.unwrap() == mock_addr, 'assertion failed');
        assert(some_number.is_some(), 'number is some');
        assert(some_number.unwrap() == index.into(), 'assertion failed');
...
fn test_get_index_pass() {
    ...
    assert(contract.do_get_index(0) == (mock_addr, 100), 'assertion failed');
    assert(contract.do_get_index(1) == (mock_addr, 200), 'assertion failed');
    assert(contract.do_get_index(2) == (mock_addr, 300), 'assertion failed');
}
...
fn test_set_pass() {
    ...
    assert(contract.do_get_index(0) == (diff_addr, 100), 'assertion failed');
    assert(contract.do_get_index(1) == (mock_addr, 20), 'assertion failed');
    assert(contract.do_get_index(2) == (diff_addr, 300), 'assertion failed');
    assert(contract.do_get_len() == (3, 3), 'assertion failed');
}
...
fn test_pop_front_pass() {
    ...
    assert(pop_addr.unwrap() == mock_addr, 'assertion failed');
    assert(pop_number.is_some(), 'pop number 2 is some');
    assert(pop_number.unwrap() == 300, 'assertion failed');
    assert(contract.do_get_len() == (2, 2), 'assertion failed');
    ...
    assert(pop_number.unwrap() == 200, 'assertion failed');
    assert(contract.do_get_len() == (1, 1), 'assertion failed');

    let (pop_addr, pop_number) = contract.do_pop_front();
    assert(pop_addr.is_some(), 'pop addr 0 is some');
    assert(pop_addr.unwrap() == mock_addr, 'assertion failed');
    assert(pop_number.is_some(), 'pop number 0 is some');
    assert(pop_number.unwrap() == 100, 'assertion failed');
    assert(contract.do_get_len() == (0, 0), 'assertion failed');
}
...
fn test_pop_append() {
    ...
    assert(addr == diff_addr, 'assertion failed');
    assert(number == 9000, 'assertion failed');
}
...
fn test_array_pass() {
    ...
    assert((array_addr.len(), array_number.len()) == contract.do_get_len(), 'assertion failed');
    assert((*array_addr[0], *array_number[0]) == contract.do_get_index(0), 'assertion failed');
    assert((*array_addr[1], *array_number[1]) == contract.do_get_index(1), 'assertion failed');
    assert((*array_addr[2], *array_number[2]) == contract.do_get_index(2), 'assertion failed');
}
...
fn test_array_empty() {
    ...
    let (array_addr, array_number) = contract.do_array();
    assert((array_addr.len(), array_number.len()) == (0, 0), 'assertion failed');
}
...
fn test_array_get_value_after_clean() {
    ...
    assert(addr.unwrap() == mock_addr, 'assertion failed');
    assert(number.is_some(), 'number is some');
    assert(number.unwrap() == 100, 'assertion failed');
...
fn test_append_span_existing_list() {
    ...
    assert(contract.do_append(mock_addr, 20) == (1, 1), 'assertion failed');
    assert(contract.do_get_len() == (2, 2), 'assertion failed');
    assert(contract.do_get_index(0) == (mock_addr, 10), 'assertion failed');
    assert(contract.do_get_index(1) == (mock_addr, 20), 'assertion failed');

    contract.do_append_span(array![mock_addr], array![30]);
    let (a, b) = contract.do_get_len();
    assert((a, b) == (3, 3), 'assertion failed');

    assert(contract.do_get_index(0) == (mock_addr, 10), 'assertion failed');
    assert(contract.do_get_index(1) == (mock_addr, 20), 'assertion failed');
    assert(contract.do_get_index(2) == (mock_addr, 30), 'assertion failed');
}
