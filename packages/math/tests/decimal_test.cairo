...
fn test_decimal_construction_from_int() {
    ...
    assert(zero.int_part() == 0, 'assertion failed');
    assert(zero.frac_part() == 0, 'assertion failed');

    let small = DecimalTrait::from_int(5);
    assert(small.int_part() == 5, 'assertion failed');
    assert(small.frac_part() == 0, 'assertion failed');

    let large = DecimalTrait::from_int(999999);
    assert(large.int_part() == 999999, 'assertion failed');
    assert(large.frac_part() == 0, 'assertion failed');
}
...
fn test_decimal_construction_from_parts() {
    ...
    assert(simple.int_part() == 10, 'assertion failed');
    assert(simple.frac_part() == 0, 'assertion failed');

    let quarter = DecimalTrait::from_parts(5, 25); // 5.25
    assert(quarter.int_part() == 5, 'assertion failed');
    // 0.25 = 1/4 of DECIMAL_SCALE = DECIMAL_SCALE/4
    let expected_frac = DECIMAL_SCALE / 4;
    assert(quarter.frac_part() == expected_frac.try_into().unwrap(), 'assertion failed');

    let half = DecimalTrait::from_parts(42, 5); // 42.5
    assert(half.int_part() == 42, 'assertion failed');
    // 0.5 = 1/2 of DECIMAL_SCALE = DECIMAL_SCALE/2
    let expected_half_frac = DECIMAL_SCALE / 2;
    assert(half.frac_part() == expected_half_frac.try_into().unwrap(), 'assertion failed');

    let decimal_places = DecimalTrait::from_parts(3, 14); // 3.14
    assert(decimal_places.int_part() == 3, 'assertion failed');
    // 0.14 = 14/100 of DECIMAL_SCALE
    let expected_14_frac = (14 * DECIMAL_SCALE) / 100;
    assert(decimal_places.frac_part() == expected_14_frac.try_into().unwrap(), 'assertion failed');

    let three_decimals = DecimalTrait::from_parts(56, 678); // 56.678
    assert(three_decimals.int_part() == 56, 'assertion failed');
    // 0.678 = 678/1000 of DECIMAL_SCALE
    let expected_678_frac = (678 * DECIMAL_SCALE) / 1000;
    assert(three_decimals.frac_part() == expected_678_frac.try_into().unwrap(), 'assertion failed');
}
...
fn test_decimal_basic_operations_integration() {
    ...
    assert(sum.int_part() == 15, 'assertion failed');

    let diff = dec1.sub(@dec2);
    assert(diff.int_part() == 5, 'assertion failed');
}
...
fn test_decimal_string_conversion_integration() {
    ...
    assert(string_repr.len() > 0, 'assertion failed');
}
...
fn test_decimal_construction_from_felt() {
    ...
    assert(zero_felt.int_part() == 0, 'assertion failed');

    let small_felt = DecimalTrait::from_felt(25);
    assert(small_felt.int_part() == 25, 'assertion failed');
    assert(small_felt.frac_part() == 0, 'assertion failed');

    let large_felt = DecimalTrait::from_felt(12345);
    assert(large_felt.int_part() == 12345, 'assertion failed');
    assert(large_felt.frac_part() == 0, 'assertion failed');
}
...
fn test_decimal_to_felt_conversion() {
    ...
    assert(integer_dec.to_felt() == 100, 'assertion failed');

    let fractional_dec = DecimalTrait::from_parts(42, 75); // 42.75
    assert(fractional_dec.to_felt() == 42, 'assertion failed'); // Truncates fractional part

    let zero_dec = DecimalTrait::from_int(0);
    assert(zero_dec.to_felt() == 0, 'assertion failed');
}
...
fn test_decimal_addition() {
    ...
    let sum = a.add(@b);
    assert(sum.int_part() == 15, 'assertion failed');
    assert(sum.frac_part() == 0, 'assertion failed');
    ...
    assert(frac_sum.int_part() == 9, 'assertion failed'); // 5.25 + 3.75 = 9.0
    assert(
        frac_sum.frac_part() == 0, 'assertion failed',
    ); // Should be exactly 9.0 with no fractional part
    ...
    assert(frac_sum2.int_part() == 3, 'assertion failed'); // 2.5 + 1.25 = 3.75
    // 0.75 = 3/4 of DECIMAL_SCALE
    let expected_75_frac = (3 * DECIMAL_SCALE) / 4;
    assert(frac_sum2.frac_part() == expected_75_frac.try_into().unwrap(), 'assertion failed');

    // Test zero addition
    let zero = DecimalTrait::from_int(0);
    let num = DecimalTrait::from_int(42);
    let zero_sum = num.add(@zero);
    assert(zero_sum.int_part() == 42, 'assertion failed');
    assert(zero_sum.frac_part() == 0, 'assertion failed');
}
...
fn test_decimal_subtraction() {
    ...
    let diff = a.sub(@b);
    assert(diff.int_part() == 7, 'assertion failed');
    assert(diff.frac_part() == 0, 'assertion failed');
    ...
    assert(frac_diff.int_part() == 3, 'assertion failed'); // 5.75 - 2.25 = 3.5
    // 0.5 = 1/2 of DECIMAL_SCALE
    let expected_half_frac = DECIMAL_SCALE / 2;
    assert(frac_diff.frac_part() == expected_half_frac.try_into().unwrap(), 'assertion failed');
    ...
    assert(carry_diff.int_part() == 6, 'assertion failed'); // 10.25 - 3.75 = 6.5
    let expected_half_frac2 = DECIMAL_SCALE / 2;
    assert(carry_diff.frac_part() == expected_half_frac2.try_into().unwrap(), 'assertion failed');

    // Test subtraction with same values
    let c = DecimalTrait::from_int(25);
    let same_diff = c.sub(@c);
    assert(same_diff.int_part() == 0, 'assertion failed');
    assert(same_diff.frac_part() == 0, 'assertion failed');

    // Test zero subtraction
    let num = DecimalTrait::from_int(100);
    let zero = DecimalTrait::from_int(0);
    let zero_diff = num.sub(@zero);
    assert(zero_diff.int_part() == 100, 'assertion failed');
}
...
fn test_decimal_multiplication() {
    ...
    assert(zero_product.int_part() == 0, 'assertion failed');
    assert(zero_product.frac_part() == 0, 'assertion failed');

    // Test multiplication by one
    let one = DecimalTrait::from_int(1);
    let one_product = num.mul(@one);
    assert(one_product.int_part() == 5, 'assertion failed');
    assert(one_product.frac_part() == 0, 'assertion failed');

    // Test basic integer multiplication
    let a = DecimalTrait::from_int(3);
    let b = DecimalTrait::from_int(4);
    let product = a.mul(@b);
    assert(product.int_part() == 12, 'assertion failed');
    assert(product.frac_part() == 0, 'assertion failed');
    ...
    assert(frac_product.int_part() == 7, 'assertion failed'); // 2.5 * 3 = 7.5
    // 0.5 = 1/2 of DECIMAL_SCALE
    let expected_half = DECIMAL_SCALE / 2;
    assert(frac_product.frac_part() == expected_half.try_into().unwrap(), 'assertion failed');
    ...
    assert(frac_frac_product.int_part() == 3, 'assertion failed'); // 1.5 * 2 = 3.0
    assert(frac_frac_product.frac_part() == 0, 'assertion failed');
}
...
fn test_decimal_division() {
    ...
    assert(quotient.int_part() == 2, 'assertion failed');
    assert(quotient.frac_part() == 0, 'assertion failed');
    ...
    assert(frac_quotient.int_part() == 2, 'assertion failed'); // 5 / 2 = 2.5
    // 0.5 = 1/2 of DECIMAL_SCALE
    let expected_half = DECIMAL_SCALE / 2;
    assert(frac_quotient.frac_part() == expected_half.try_into().unwrap(), 'assertion failed');
    ...
    assert(frac_frac_quotient.int_part() == 3, 'assertion failed'); // 7.5 / 2.5 = 3.0
    assert(frac_frac_quotient.frac_part() == 0, 'assertion failed');

    // Test division by one
    let g = DecimalTrait::from_parts(12, 34); // 12.34
    let one = DecimalTrait::from_int(1);
    let div_by_one = g.div(@one);
    assert(div_by_one.int_part() == 12, 'assertion failed');
    // Should preserve fractional part: 0.34 = 34/100 of DECIMAL_SCALE
    let expected_34_frac = (34 * DECIMAL_SCALE) / 100;
    assert(div_by_one.frac_part() == expected_34_frac.try_into().unwrap(), 'assertion failed');

    // Test division by zero (should return 0)
    let h = DecimalTrait::from_int(10);
    let zero = DecimalTrait::from_int(0);
    let div_by_zero = h.div(@zero);
    assert(div_by_zero.int_part() == 0, 'assertion failed');
    assert(div_by_zero.frac_part() == 0, 'assertion failed');
}
...
fn test_decimal_operator_overloads() {
    ...
    let sum = a + b;
    assert(sum.int_part() == 9, 'assertion failed');

    // Test subtraction operator
    let diff = a - b;
    assert(diff.int_part() == 3, 'assertion failed');

    // Test multiplication and division operators (now fixed)
    let product = a * b;
    assert(product.int_part() == 18, 'assertion failed');
    assert(product.frac_part() == 0, 'assertion failed');
...
fn test_decimal_chained_operations() {
    ...
    // Test 4 + (2 + 2) = 8
    let result2 = a + (b + c);
    assert(result2.int_part() == 8, 'assertion failed');

    // Test (4 + 2) - 2 = 4
    let result3 = (a + b) - c;
    assert(result3.int_part() == 4, 'assertion failed');

    // Test multiplication and division in chains (now fixed)
    let result1 = (a + b) / c; // (4 + 2) / 2 = 3
    assert(result1.int_part() == 3, 'assertion failed');
    assert(result1.frac_part() == 0, 'assertion failed');

    let result4 = (a - b) * c; // (4 - 2) * 2 = 4
    assert(result4.int_part() == 4, 'assertion failed');
    assert(result4.frac_part() == 0, 'assertion failed');
}
...
fn test_decimal_precision_boundaries() {
    ...
    assert(large_int.int_part() == 18446744073709551615, 'assertion failed');
    assert(large_int.frac_part() == 0, 'assertion failed');

    let half = DecimalTrait::from_parts(100, 5); // 100.5
    assert(half.int_part() == 100, 'assertion failed');

    let quarter = DecimalTrait::from_parts(0, 25); // 0.25
    assert(quarter.int_part() == 0, 'assertion failed');
}
...
fn test_decimal_string_parsing_edge_cases() {
    ...
    assert(invalid1.is_none(), 'assertion failed');

    let invalid2 = DecimalTrait::from_string("");
    assert(invalid2.is_none(), 'assertion failed');

    let invalid3 = DecimalTrait::from_string("12.34.56");
    assert(invalid3.is_none(), 'assertion failed');

    // Test valid edge cases
    let single_digit = DecimalTrait::from_string("5");
    assert(single_digit.is_some(), 'assertion failed');
    assert(single_digit.unwrap().int_part() == 5, 'assertion failed');
}
...
fn test_decimal_internal_representation() {
    ...
    let zero = DecimalTrait::from_int(0);
    assert(zero.int_part == 0, 'assert');
    assert(zero.frac_part == 0, 'assert');

    let one = DecimalTrait::from_int(1);
    assert(one.int_part == 1, 'assert');
    assert(one.frac_part == 0, 'assert');

    // Test internal representation with raw parts for low-level testing
    let fractional = DecimalTrait::from_raw_parts(0, 1);
    assert(fractional.int_part == 0, 'assert');
    assert(fractional.frac_part == 1, 'assert');

    let mixed = DecimalTrait::from_raw_parts(1, 1);
    assert(mixed.int_part == 1, 'assert');
    assert(mixed.frac_part == 1, 'assert');
}
...
fn test_comprehensive_fractional_verification() {
    ...
    assert(one_tenth.frac_part() == expected_1_frac.try_into().unwrap(), 'assertion failed');

    let nine_tenths = DecimalTrait::from_parts(0, 9); // 0.9
    let expected_9_frac = (9 * DECIMAL_SCALE) / 10;
    assert(nine_tenths.frac_part() == expected_9_frac.try_into().unwrap(), 'assertion failed');

    // Test two decimal places
    let quarter = DecimalTrait::from_parts(0, 25); // 0.25
    let expected_25_frac = DECIMAL_SCALE / 4;
    assert(quarter.frac_part() == expected_25_frac.try_into().unwrap(), 'assertion failed');

    let seventy_five = DecimalTrait::from_parts(0, 75); // 0.75
    let expected_75_frac = (3 * DECIMAL_SCALE) / 4;
    assert(seventy_five.frac_part() == expected_75_frac.try_into().unwrap(), 'assertion failed');

    // Test three decimal places
    let one_eighth = DecimalTrait::from_parts(0, 125); // 0.125
    let expected_125_frac = DECIMAL_SCALE / 8;
    assert(one_eighth.frac_part() == expected_125_frac.try_into().unwrap(), 'assertion failed');

    let three_eighths = DecimalTrait::from_parts(0, 375); // 0.375
    let expected_375_frac = (3 * DECIMAL_SCALE) / 8;
    assert(three_eighths.frac_part() == expected_375_frac.try_into().unwrap(), 'assertion failed');

    // Test with integer parts
    let pi_approx = DecimalTrait::from_parts(3, 14159); // 3.14159
    assert(pi_approx.int_part() == 3, 'assertion failed');
    let expected_pi_frac = (14159 * DECIMAL_SCALE) / 100000;
    assert(pi_approx.frac_part() == expected_pi_frac.try_into().unwrap(), 'assertion failed');

    let e_approx = DecimalTrait::from_parts(2, 71828); // 2.71828
    assert(e_approx.int_part() == 2, 'assertion failed');
    let expected_e_frac = (71828 * DECIMAL_SCALE) / 100000;
    assert(e_approx.frac_part() == expected_e_frac.try_into().unwrap(), 'assertion failed');
}
...
fn test_decimal_performance_operations() {
    ...
    let sum = a + b;
    assert(sum.int_part() == 75, 'assertion failed');

    let diff = a - b;
    assert(diff.int_part() == 25, 'assertion failed');

    // Test multiplication and division (now fixed)
    let product = DecimalTrait::from_int(5) * DecimalTrait::from_int(3);
    assert(product.int_part() == 15, 'assertion failed');
    assert(product.frac_part() == 0, 'assertion failed');

    let quotient = DecimalTrait::from_int(50) / DecimalTrait::from_int(5);
    assert(quotient.int_part() == 10, 'assertion failed');
    assert(quotient.frac_part() == 0, 'assertion failed');
}
...
fn test_decimal_mathematical_properties() {
    ...
    assert((a + zero) == a, 'assertion failed');

    // Test commutativity of addition: a + b = b + a
    assert((a + b) == (b + a), 'assertion failed');

    // Test subtraction: a - a = 0
    assert((a - a) == zero, 'assertion failed');

    // Test multiplication properties (now fixed)
    let one = DecimalTrait::from_int(1);
    assert((a * one) == a, 'assertion failed'); // Multiplicative identity
    assert((a * b) == (b * a), 'assertion failed'); // Commutativity of multiplication

    // Test multiplicative identity with fractions
    let frac = DecimalTrait::from_parts(3, 14); // 3.14
    assert((frac * one) == frac, 'assertion failed');

    // Test division properties
    assert((a / one) == a, 'assertion failed'); // Division by one
    assert((a / a).int_part() == 1, 'assertion failed'); // Self division
    assert((a / a).frac_part() == 0, 'assertion failed');
}
...
fn test_signed_decimal_construction_from_parts() {
    ...
    assert(positive.int_part() == 10, 'assertion failed');
    assert(positive.is_negative() == false, 'assertion failed');
    let expected_25_frac = DECIMAL_SCALE / 4; // 0.25 = 1/4
    assert(positive.frac_part() == expected_25_frac.try_into().unwrap(), 'assertion failed');

    // Test creating negative signed decimals
    let negative = DecimalTrait::from_parts_signed(10, 25, true); // -10.25
    assert(negative.int_part() == 10, 'assertion failed');
    assert(negative.is_negative() == true, 'assertion failed');
    assert(negative.frac_part() == expected_25_frac.try_into().unwrap(), 'assertion failed');

    // Test creating negative zero
    let negative_zero = DecimalTrait::from_parts_signed(0, 0, true); // -0.0
    assert(negative_zero.int_part() == 0, 'assertion failed');
    assert(negative_zero.frac_part() == 0, 'assertion failed');
    assert(negative_zero.is_negative() == true, 'assertion failed');
}
...
fn test_signed_decimal_construction_from_raw_parts() {
    ...
    assert(positive.int_part() == 5, 'assertion failed');
    assert(positive.frac_part() == 100, 'assertion failed');
    assert(positive.is_negative() == false, 'assertion failed');

    let negative = DecimalTrait::from_raw_parts_signed(5, 100, true);
    assert(negative.int_part() == 5, 'assertion failed');
    assert(negative.frac_part() == 100, 'assertion failed');
    assert(negative.is_negative() == true, 'assertion failed');
}
...
fn test_signed_decimal_construction_from_felt() {
    ...
    assert(positive.int_part() == 42, 'assertion failed');
    assert(positive.frac_part() == 0, 'assertion failed');
    assert(positive.is_negative() == false, 'assertion failed');

    let negative = DecimalTrait::from_felt_signed(42, true);
    assert(negative.int_part() == 42, 'assertion failed');
    assert(negative.frac_part() == 0, 'assertion failed');
    assert(negative.is_negative() == true, 'assertion failed');

    // Test zero with different signs
    let positive_zero = DecimalTrait::from_felt_signed(0, false);
    assert(positive_zero.is_negative() == false, 'assertion failed');
...
fn test_signed_decimal_to_felt_conversion() {
    ...
    assert(positive.to_felt() == 42, 'assertion failed'); // Truncates fractional part

    let negative = DecimalTrait::from_parts_signed(42, 75, true); // -42.75
    assert(negative.to_felt() == -42, 'assertion failed'); // Should be negative

    let positive_zero = DecimalTrait::from_parts_signed(0, 0, false);
    assert(positive_zero.to_felt() == 0, 'assertion failed');

    let negative_zero = DecimalTrait::from_parts_signed(0, 0, true);
    assert(negative_zero.to_felt() == 0, 'assertion failed'); // -0 converts to 0
}
...
fn test_signed_decimal_addition_mixed_signs() {
    ...
    assert(result1.int_part() == 5, 'assertion failed');
    assert(result1.is_negative() == false, 'assertion failed');
    let expected_25_frac = DECIMAL_SCALE / 4;
    assert(result1.frac_part() == expected_25_frac.try_into().unwrap(), 'assertion failed');

    // (-5.25) + 10.5 = 5.25
    let result2 = neg_b.add(@pos_a);
    assert(result2.int_part() == 5, 'assertion failed');
    assert(result2.is_negative() == false, 'assertion failed');
    assert(result2.frac_part() == expected_25_frac.try_into().unwrap(), 'assertion failed');
    ...
    assert(result3.int_part() == 5, 'assertion failed');
    assert(result3.is_negative() == true, 'assertion failed');
    assert(result3.frac_part() == expected_25_frac.try_into().unwrap(), 'assertion failed');
}
...
fn test_signed_decimal_addition_same_signs() {
    ...
    assert(result.int_part() == 9, 'assertion failed');
    assert(result.frac_part() == 0, 'assertion failed');
    assert(result.is_negative() == true, 'assertion failed');
    ...
    assert(result2.int_part() == 9, 'assertion failed');
    assert(result2.frac_part() == 0, 'assertion failed');
    assert(result2.is_negative() == false, 'assertion failed');
}
...
fn test_signed_decimal_subtraction() {
    ...
    assert(result1.int_part() == 15, 'assertion failed');
    assert(result1.frac_part() == 0, 'assertion failed');
    assert(result1.is_negative() == false, 'assertion failed');

    // (-5.0) - 10.0 = (-5.0) + (-10.0) = -15.0
    let result2 = neg_b.sub(@pos_a);
    assert(result2.int_part() == 15, 'assertion failed');
    assert(result2.frac_part() == 0, 'assertion failed');
    assert(result2.is_negative() == true, 'assertion failed');

    // Test subtracting same negative numbers: (-5.0) - (-5.0) = 0.0
    let result3 = neg_b.sub(@neg_b);
    assert(result3.int_part() == 0, 'assertion failed');
    assert(result3.frac_part() == 0, 'assertion failed');
    assert(result3.is_negative() == false, 'assertion failed');
}
...
fn test_signed_decimal_multiplication() {
    ...
    assert(result1.int_part() == 15, 'assertion failed');
    assert(result1.frac_part() == 0, 'assertion failed');
    assert(result1.is_negative() == true, 'assertion failed');
    ...
    assert(result3.int_part() == 6, 'assertion failed');
    assert(result3.frac_part() == 0, 'assertion failed');
    assert(result3.is_negative() == false, 'assertion failed');
    ...
    assert(result4.int_part() == 10, 'assertion failed');
    assert(result4.frac_part() == 0, 'assertion failed');
    assert(result4.is_negative() == true, 'assertion failed');
}
...
fn test_signed_decimal_division() {
    ...
    assert(result4.int_part() == 2, 'assertion failed');
    assert(result4.is_negative() == true, 'assertion failed');
    let expected_half = DECIMAL_SCALE / 2; // 0.5
    assert(result4.frac_part() == expected_half.try_into().unwrap(), 'assertion failed');
}
...
fn test_signed_decimal_string_representation() {
    ...
    assert(small_frac_str == "0.5", 'assert');
    ...
    assert(large_str == "999999.123456789", 'assert');
}
...
fn test_signed_decimal_operator_overloads() {
    ...
    let sum = pos_a + neg_b;
    assert(sum.int_part() == 7, 'assertion failed');
    assert(sum.frac_part() == 0, 'assertion failed');
    assert(sum.is_negative() == false, 'assertion failed');

    // Test subtraction operator: 10.0 - (-3.0) = 13.0
    let diff = pos_a - neg_b;
    assert(diff.int_part() == 13, 'assertion failed');
    assert(diff.frac_part() == 0, 'assertion failed');
    assert(diff.is_negative() == false, 'assertion failed');

    // Test multiplication operator: 10.0 * (-3.0) = -30.0
    let product = pos_a * neg_b;
    assert(product.int_part() == 30, 'assertion failed');
    assert(product.frac_part() == 0, 'assertion failed');
    assert(product.is_negative() == true, 'assertion failed');

    // Test division operator: 10.0 / (-3.0) = -3.333... (truncated)
    let quotient = pos_a / neg_b;
    assert(quotient.int_part() == 3, 'assertion failed');
    assert(quotient.is_negative() == true, 'assertion failed');
}
...
fn test_signed_decimal_edge_cases() {
    ...
    assert(tiny_neg.int_part() == 0, 'assertion failed');
    assert(tiny_neg.is_negative() == true, 'assertion failed');
    assert(tiny_neg.frac_part() > 0, 'assertion failed');
}
...
fn test_signed_decimal_equality() {
    ...
    assert(pos_a == pos_b, 'assert');

    // Positive and negative versions should not be equal
    assert(!(pos_a == neg_c), 'assertion failed');
    ...
    assert(!(zero_pos == zero_neg), 'assertion failed');
}
...
fn test_signed_decimal_chained_operations() {
    ...
    let result1 = (a + b) + c;
    assert(result1.int_part() == 7, 'assertion failed');
    assert(result1.frac_part() == 0, 'assertion failed');
    assert(result1.is_negative() == false, 'assertion failed');

    // Test: 10.0 - (-5.0) * 2.0 = 10.0 - (-10.0) = 10.0 + 10.0 = 20.0
    let result2 = a - (b * c);
    assert(result2.int_part() == 20, 'assertion failed');
    assert(result2.frac_part() == 0, 'assertion failed');
    assert(result2.is_negative() == false, 'assertion failed');

    // Test: (10.0 + (-5.0)) / 2.0 = 5.0 / 2.0 = 2.5
    let result3 = (a + b) / c;
    assert(result3.int_part() == 2, 'assertion failed');
    assert(result3.is_negative() == false, 'assertion failed');
    let expected_half = DECIMAL_SCALE / 2;
    assert(result3.frac_part() == expected_half.try_into().unwrap(), 'assertion failed');
}
