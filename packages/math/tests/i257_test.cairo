...
fn i257_test_add() {
    ...
    let result = a + b;
    assert(result.abs() == 55, 'assertion failed');
    ...
    let result = a + b;
    assert(result.abs() == 0, 'assertion failed');
    ...
    let b = I257Impl::new(13, true);
    let result = a + b;
    assert(result.abs() == 29, 'assertion failed');
    assert(!result.is_negative(), '42 - 13 -> positive');
...
fn i257_test_sub() {
    ...
    let b = I257Impl::new(42, true);
    let result = a - b;
    assert(result.abs() == 84, 'assertion failed');
    ...
    let b = I257Impl::new(42, true);
    let result = a - b;
    assert(result.abs() == 142, 'assertion failed');
...
fn i257_test_mul() {
    ...
    let result = a * b;
    assert(result.abs() == 50, 'assertion failed');
...
fn i257_test_div_no_rem() {
    ...
    let b = I257Impl::new(5, false);
    let result = a / b;
    assert(result.abs() == 2, 'assertion failed');
    assert(!result.is_negative(), '10 // 5 -> positive');
...
fn i257_test_div_sign_zero() {
    ...
    assert(x.abs() == 0, 'assertion failed');
...
fn i257_test_into() {
    let x: i257 = 35.into();
    assert(x.abs() == 35, 'assertion failed');
    assert(!x.is_negative(), 'incorrect into sign');

    let y: i257 = 258973.into();
    assert(y.abs() == 258973, 'assertion failed');
