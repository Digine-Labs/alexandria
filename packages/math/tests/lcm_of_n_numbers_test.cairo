...
fn lcm_test() {
    ...
    assert(lcm(arr.span()).unwrap() == 120, 'assertion failed');
}
...
fn lcm_test_3() {
    ...
    assert(lcm(arr.span()).unwrap() == 396, 'assertion failed');
}

#[test]
fn lcm_test_4() {
    let arr = array![1_u128, 2, 8, 3];
    assert(lcm(arr.span()).unwrap() == 24, 'assertion failed');
}
...
fn lcm_single_test() {
    ...
    assert(lcm(arr.span()).unwrap() == 10, 'assertion failed');
}
