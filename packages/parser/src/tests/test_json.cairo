use alexandria_parser::json::{JsonError, JsonValue, parse_json};
...
#[test]
fn test_decimal_numbers_not_supported() {
    // Current implementation limitation: only supports integers
    // It will parse "3" from "3.14" and consider it valid (incorrectly)
    let result1 = parse_json("3.14");
    assert!(result1.is_ok());

    if let JsonValue::Number(n) = result1.unwrap() {
        assert!(n == 3); // Only parsed "3", ignored ".14"
    } else {
        panic!("Expected number");
    }

    // This demonstrates the parser doesn't validate full input consumption
    // Both of these incorrectly succeed when they should fail
    let result2 = parse_json("42.5");
    assert!(result2.is_ok());
    if let JsonValue::Number(n) = result2.unwrap() {
        assert!(n == 42); // Only parsed "42", ignored ".5"
    }
}
