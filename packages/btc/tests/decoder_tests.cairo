...
fn test_compact_size_decoding() {
    ...
    assert(result == 0x42, 'assert failed');
    ...
    let mut decoder = TransactionDecoderTrait::new(data);
    let result = decoder.read_compact_size();
    assert(result == 0x1234, 'assert failed');
    ...
    let mut decoder = TransactionDecoderTrait::new(data);
    let result = decoder.read_compact_size();
    assert(result == 0x12345678, 'assert failed');
}
...
fn test_u64_le_reading() {
    ...
    assert(result == 0x1122334455667788, 'assert failed');
}
...
fn test_legacy_transaction_decoding() {
    ...
    assert(tx.version == 1, 'assert failed');
    assert(tx.inputs.len() == 1, 'inputs len should be 1');
    assert(tx.outputs.len() == 1, 'outputs len should be 1');
    assert(tx.locktime == 0, 'assert failed');
    assert(!tx.is_segwit, 'assert failed');

    // Check input
    let input = tx.inputs.at(0);
    assert(*input.previous_vout == 0, 'assert failed');
    assert(*input.sequence == 0xffffffff, 'assert failed');
    assert(input.script_sig.len() == 0, 'len should be 0');

    // Check output
    let output = tx.outputs.at(0);
    assert(*output.value == 1, 'assert failed');
    assert(output.script_pubkey.len() == 25, 'len should be 25');
}
...
fn test_segwit_transaction_detection() {
    ...
    assert(tx.is_segwit, 'assert failed');
    assert(tx.witness.len() == tx.inputs.len(), 'witness len eq inputs len');
}
...
fn test_witness_data_decoding() {
    ...
    assert(witness.witness_stack.len() == 2, 'len should be 2');
    assert(witness.witness_stack.at(0).len() == 71, 'len should be 71');
    assert(witness.witness_stack.at(1).len() == 33, 'len should be 33');
}
...
fn test_multi_input_output_transaction() {
    ...
    assert(tx.inputs.len() == 2, 'len should be 2');
    assert(tx.outputs.len() == 2, 'outputs len should be 2');

    // Check first output value
    assert(*tx.outputs.at(0).value == 1, 'value should be 1');
    // Check second output value
    assert(*tx.outputs.at(1).value == 2, 'value should be 2');
}
...
fn test_real_bitcoin_transaction_decoding() {
    ...
    assert(*input1.previous_vout == 1, 'assert failed');
    assert(*input1.sequence == 0xffffffff, 'assert failed');
    assert(input1.script_sig.len() == 138, 'len should be 138');
    assert(input1.previous_txid.len() == 32, 'len should be 32');

    // Verify first input script signature content (first few bytes)
    assert(input1.script_sig.at(0).unwrap() == 0x47, 'should be 0x47'); // DER signature length
    assert(input1.script_sig.at(1).unwrap() == 0x30, 'should be 0x30'); // DER sequence tag
    assert(input1.script_sig.at(2).unwrap() == 0x44, 'should be 0x44'); // DER sequence length
    assert(input1.script_sig.at(3).unwrap() == 0x02, 'should be 0x02'); // DER integer tag (r value)
    assert(input1.script_sig.at(4).unwrap() == 0x20, 'should be 0x20'); // r value length (32 bytes)

    // Verify second input
    let input2 = tx.inputs.at(1);
    assert(*input2.previous_vout == 1, 'assert failed');
    assert(*input2.sequence == 0xffffffff, 'assert failed');
    assert(input2.script_sig.len() == 138, 'len should be 138');
    assert(input2.previous_txid.len() == 32, 'len should be 32');

    // Verify second input script signature content (first few bytes)
    assert(input2.script_sig.at(0).unwrap() == 0x47, 'should be 0x47'); // DER signature length
    assert(input2.script_sig.at(1).unwrap() == 0x30, 'should be 0x30'); // DER sequence tag
    assert(input2.script_sig.at(2).unwrap() == 0x44, 'should be 0x44'); // DER sequence length
    assert(input2.script_sig.at(3).unwrap() == 0x02, 'should be 0x02'); // DER integer tag (r value)
    assert(input2.script_sig.at(4).unwrap() == 0x20, 'should be 0x20'); // r value length (32 bytes)
    ...
    assert(input1.script_sig.at(137).unwrap() == 0x7a, 'should be 0x7a'); // Last byte of public key
    assert(input2.script_sig.at(137).unwrap() == 0x7a, 'should be 0x7a'); // Last byte of public key
    assert(input1.script_sig.at(136).unwrap() == 0x04, 'should be 0x04'); // Second to last byte
    assert(input2.script_sig.at(136).unwrap() == 0x04, 'should be 0x04'); // Second to last byte

    // Verify first output (1420100 satoshis)
    let output1 = tx.outputs.at(0);
    assert(*output1.value == 1420100, 'assert failed');
    assert(output1.script_pubkey.len() == 25, 'len should be 25');

    // Verify second output (69239 satoshis)
    let output2 = tx.outputs.at(1);
    assert(*output2.value == 69239, 'assert failed');
    assert(output2.script_pubkey.len() == 25, 'len should be 25');
}
...
fn test_compact_size_edge_cases() {
    ...
    let mut decoder = TransactionDecoderTrait::new(data);
    let result = decoder.read_compact_size();
    assert(result == 0xfc, 'assert failed');
    ...
    let mut decoder = TransactionDecoderTrait::new(data);
    let result = decoder.read_compact_size();
    assert(result == 0xfd, 'assert failed');
    ...
    let mut decoder = TransactionDecoderTrait::new(data);
    let result = decoder.read_compact_size();
    assert(result == 0xffff, 'assert failed');
    ...
    let mut decoder = TransactionDecoderTrait::new(data);
    let result = decoder.read_compact_size();
    assert(result == 0x123456789abcdef0, 'assert failed');
}
...
fn test_empty_transaction_components() {
    ...
    assert(tx.version == 1, 'assert failed');
    assert(tx.inputs.len() == 0, 'len should be 0');
    assert(tx.outputs.len() == 0, 'outputs len should be 0');
    assert(tx.locktime == 0, 'assert failed');
    assert(!tx.is_segwit, 'assert failed');
}
...
fn test_maximum_values() {
    ...
    assert(tx.version == 0xffffffff, 'assert failed');
    assert(tx.inputs.len() == 1, 'inputs len should be 1');
    assert(*tx.inputs.at(0).previous_vout == 0xffffffff, 'previous vout should match');
    assert(*tx.inputs.at(0).sequence == 0xffffffff, 'sequence should match');
    assert(tx.outputs.len() == 1, 'outputs len should be 1');
    assert(*tx.outputs.at(0).value == 0xffffffffffffffff, 'output value should match');
    assert(tx.locktime == 0xffffffff, 'assert failed');
}
...
fn test_large_script_sizes() {
    ...
    assert(tx.inputs.at(0).script_sig.len() == 252, 'len should be 252');
    assert(tx.outputs.at(0).script_pubkey.len() == 252, 'len should be 252');

    // Verify script content
    assert(tx.inputs.at(0).script_sig.at(0).unwrap() == 0x6a, 'should be 0x6a');
    assert(tx.inputs.at(0).script_sig.at(251).unwrap() == 0x6a, 'should be 0x6a');
    assert(tx.outputs.at(0).script_pubkey.at(0).unwrap() == 0x51, 'should be 0x51');
    assert(tx.outputs.at(0).script_pubkey.at(251).unwrap() == 0x51, 'should be 0x51');
}
