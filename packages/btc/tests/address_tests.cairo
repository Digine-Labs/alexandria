...
fn test_create_private_key() {
    ...
    assert(
        private_key.key == 0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef,
        'assert failed',
    );
    assert(private_key.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert(private_key.compressed, 'assert failed');
}
...
fn test_generate_p2pkh_address() {
    ...
    assert(address.address_type == BitcoinAddressType::P2PKH, 'assert failed');
    assert(address.network == BitcoinNetwork::Mainnet, 'assert failed');
...
fn test_generate_p2sh_address() {
    ...
    assert(address.address_type == BitcoinAddressType::P2SH, 'assert failed');
    assert(address.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert!(address.address.len() > 0);
    assert!(address.script_pubkey.len() > 0);
}
...
fn test_generate_p2wpkh_address() {
    ...
    assert(address.address_type == BitcoinAddressType::P2WPKH, 'assert failed');
    assert(address.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert!(address.address.len() > 0);
    assert!(address.script_pubkey.len() > 0);
}
...
fn test_generate_p2tr_address() {
    ...
    assert(address.address_type == BitcoinAddressType::P2TR, 'assert failed');
    assert(address.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert!(address.address.len() > 0);
    assert!(address.script_pubkey.len() > 0);
}
...
fn test_different_networks() {
    ...
    assert(address_mainnet.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert(address_testnet.network == BitcoinNetwork::Testnet, 'assert failed');

    // Addresses should be different due to different network prefixes
    assert(address_mainnet.address != address_testnet.address, 'assert failed');
}
...
fn test_compressed_vs_uncompressed() {
    ...
    assert(address_compressed.address != address_uncompressed.address, 'assert failed');
}
...
fn test_generate_p2wsh_address() {
    ...
    assert(address.address_type == BitcoinAddressType::P2WSH, 'assert failed');
    assert(address.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert!(address.address.len() > 0);
    assert!(address.script_pubkey.len() > 0);
...
fn test_p2wsh_different_networks() {
    ...
    assert(address.address_type == BitcoinAddressType::P2WSH, 'assert failed');
    assert(address.network == BitcoinNetwork::Testnet, 'assert failed');
...
fn test_boundary_coordinate_values() {
    ...
    assert(addr_zero.address != addr_max.address, 'assert failed');
}
...
fn test_public_key_coordinate_validation() {
    ...
    assert(addr_field.address != addr_valid.address, 'assert failed');
}
...
fn test_all_address_types_consistency() {
    ...
    assert(p2pkh.address != p2sh.address, 'assert failed');
    assert(p2pkh.address != p2wpkh.address, 'assert failed');
    assert(p2pkh.address != p2wsh.address, 'assert failed');
    assert(p2pkh.address != p2tr.address, 'assert failed');
    assert(p2sh.address != p2wpkh.address, 'assert failed');
    assert(p2sh.address != p2wsh.address, 'assert failed');
    assert(p2sh.address != p2tr.address, 'assert failed');
    assert(p2wpkh.address != p2wsh.address, 'assert failed');
    assert(p2wpkh.address != p2tr.address, 'assert failed');
    assert(p2wsh.address != p2tr.address, 'assert failed');
    ...
    assert(p2pkh.address_type == BitcoinAddressType::P2PKH, 'assert failed');
    assert(p2sh.address_type == BitcoinAddressType::P2SH, 'assert failed');
    assert(p2wpkh.address_type == BitcoinAddressType::P2WPKH, 'assert failed');
    assert(p2wsh.address_type == BitcoinAddressType::P2WSH, 'assert failed');
    assert(p2tr.address_type == BitcoinAddressType::P2TR, 'assert failed');
}
