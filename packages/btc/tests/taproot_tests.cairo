...
fn test_tagged_hash() {
    ...
    assert(hash == hash2, 'Tagged hash should be determ');

    // Different tags should produce different hashes
    let hash3 = tagged_hash(tag2, data.span());
    assert(hash != hash3, 'Different tags diff hashes');
}
...
fn test_key_tweaking() {
    ...
    match tweaked_result {
        ...
        Option::None => { assert(false, 'Key tweaking should succeed'); },
    }
}
    ...
    match tweaked_result {
        Option::Some(tweaked) => {
            ...
            match key_path_result {
                ...
                Option::None => { assert(false, 'Key-path tweaking succeed'); },
            }
        },
        Option::None => { assert(false, 'Key tweaking w merkle succeed'); },
    }
}
...
fn test_u256_to_32_bytes_conversion() {
    ...
    assert(all_zero, 'All bytes should be zero for 0');
}
...
fn test_create_key_path_output() {
    ...
    match output {
        Option::Some(tweaked) => {
            assert(tweaked.output_key != internal_key, 'Key-path output tweaked');
        },
        Option::None => { assert(false, 'Key-path output succeed'); },
    }
}
...
fn test_script_tree_operations() {
    ...
    match merkle_root {
        Option::Some(root) => { assert(root > 0, 'Merkle root should be non-zero'); },
        Option::None => { assert(false, 'Should have merkle root'); },
    }
    ...
    match output {
        Option::Some(tweaked) => {
            ...
            match key_path_output {
                ...
                Option::None => { assert(false, 'Key-path output should succeed'); },
            }
        },
        Option::None => { assert(false, 'Script-path output succeed'); },
    }
}
...
fn test_taproot_signature_verification_interface() {
    ...
    assert(!result, 'Dummy sig should fail verify');
    ...
    assert(!result2, 'Should reject sig wrong length');
}
...
fn test_deterministic_tweaking() {
    ...
    match (result1, result2) {
        (
            ...
            assert(tweaked1.output_key == tweaked2.output_key, 'Tweaking should be determ');
            assert(tweaked1.parity == tweaked2.parity, 'Parity should also be determ');
        },
        _ => { assert(false, 'Both tweaking ops succeed'); },
    }
}
