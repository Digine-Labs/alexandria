...
fn test_signature_verification_with_invalid_values() {
    ...
    assert(!result, 'Arb sig vals fail verify');
}
...
fn test_signature_verification_known_generator_point() {
    ...
    assert(!result, 'Small arb vals fail verify');
}
...
fn test_signature_validation_comprehensive() {
    ...
    assert(!result1, 'Max valid r,s fail verify');
    ...
    assert(!result2, 'Min valid r,s fail verify');
    ...
    assert(!result3a, 'Even y_parity arb sig fail');
    assert(!result3b, 'Odd y_parity arb sig fail');
}
...
fn test_signature_component_validation() {
    ...
    assert(!result1, 'Sig with r=0 invalid');
    ...
    assert(!result2, 'Sig with s=0 invalid');
    ...
    assert(!result3, 'Verify with msg_hash=0 invalid');
}
...
fn test_der_signature_parsing() {
    ...
    if let Option::Some(sig) = result {
        assert(sig.r > 0, 'Parsed r should be non-zero');
        assert(sig.s > 0, 'Parsed s should be non-zero');
    }
}
...
fn test_transaction_signature_verification() {
    ...
    assert(!result, 'Arb sig should fail tx verify');
    ...
    assert(!result_sighash_none, 'Arb sig SIGHASH_NONE fail');
}
...
fn test_modular_inverse_edge_cases() {
    ...
    assert(inv0 == 0, 'Inverse of 0 should be 0');
}
...
fn test_ecdsa_verification_known_vectors() {
    ...
    assert(!result, 'Invalid sig should fail verify');
    ...
    assert(!result2, 'Sig with r=0 should fail');
    ...
    assert(!result3, 'Sig with s=0 should fail');
}
...
fn test_der_signature_verification() {
    ...
    assert(!result, 'Test DER sig should fail verify');
    ...
    assert(!result_invalid, 'Invalid DER should fail verify');
}
...
fn test_legacy_verify_addr_failure() {
    ...
    assert(!result, 'Sig verify fail invalid pub key');
}
