...
fn test_create_private_key_valid() {
    ...
    assert(private_key.key == key, 'assert failed');
    assert(private_key.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert(private_key.compressed == true, 'assert failed');
}
...
fn test_create_private_key_uncompressed() {
    ...
    assert(private_key.key == key, 'assert failed');
    assert(private_key.network == BitcoinNetwork::Testnet, 'assert failed');
    assert(private_key.compressed == false, 'assert failed');
}
...
fn test_private_key_to_public_key_compressed() {
    ...
    assert(first_byte == 0x02 || first_byte == 0x03, 'assert failed');
}
...
fn test_private_key_to_public_key_uncompressed() {
    ...
    let first_byte = public_key.bytes.at(0).unwrap();
    assert(first_byte == 0x04, 'assert failed');
}
...
fn test_different_public_keys_produce_different_hashes() {
    ...
    assert(hash1 != hash2, 'assert failed');
}
...
fn test_generate_private_key() {
    ...
    assert(private_key.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert(private_key.compressed == true, 'assert failed');
    assert(private_key.key != 0, 'assert failed');
}
...
fn test_generate_private_key_different_networks() {
    ...
    assert(mainnet_key.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert(testnet_key.network == BitcoinNetwork::Testnet, 'assert failed');

    // Both should be valid but may have same key value (deterministic implementation)
    assert(mainnet_key.key != 0, 'assert failed');
    assert(testnet_key.key != 0, 'assert failed');
}
...
fn test_public_key_x_coordinate_consistency() {
    ...
    assert(x_coord != 0, 'assert failed'); // Should be valid coordinate
}
...
fn test_public_key_bytes_consistency() {
    ...
    assert(bytes1 == bytes2, 'assert failed'); // Should be consistent
}
...
fn test_private_key_curve_boundary() {
    ...
    assert(private_key.key == near_max_key, 'assert failed');
    assert!(public_key.is_compressed());
...
fn test_private_key_one() {
    ...
    assert(private_key.key == 1, 'assert failed');
    assert!(public_key.is_compressed());

    // Should produce a valid, non-zero public key
    let x_coord = public_key.get_x_coordinate();
    assert(x_coord != 0, 'assert failed');
}
...
fn test_public_key_compression_flag_consistency() {
    ...
    assert(first_byte_compressed == 0x02 || first_byte_compressed == 0x03, 'assert failed');
    ...
    assert(first_byte_uncompressed == 0x04, 'assert failed');
}
...
fn test_generate_private_key_uniqueness() {
    ...
    assert(key1.key != 0, 'assert failed');
    assert(key2.key != 0, 'assert failed');

    // Network should be different
    assert(key1.network != key2.network, 'assert failed');
}
...
fn test_private_key_all_networks() {
    ...
    assert(mainnet_key.network == BitcoinNetwork::Mainnet, 'assert failed');
    assert(testnet_key.network == BitcoinNetwork::Testnet, 'assert failed');
    assert(regtest_key.network == BitcoinNetwork::Regtest, 'assert failed');

    // All should have same key value
    assert(mainnet_key.key == testnet_key.key, 'assert failed');
    assert(testnet_key.key == regtest_key.key, 'assert failed');
}
...
fn test_public_key_hash_deterministic() {
    ...
    assert(hash1 == hash2, 'assert failed');
    assert(hash2 == hash3, 'assert failed');
    assert!(hash1.len() == 20);
}
...
fn test_public_key_coordinates_range() {
    ...
    assert(x_coord < field_prime, 'assert failed');
    assert(x_coord != 0, 'assert failed');
}
...
fn test_public_key_to_coords_conversion() {
    ...
    assert(coords_compressed.compressed == true, 'assert failed');
    assert!(coords_compressed.x == public_key_compressed.get_x_coordinate());
    assert(coords_compressed.y == 0, 'assert failed'); // y should be 0 for compressed
    ...
    assert(coords_uncompressed.compressed == false, 'assert failed');
