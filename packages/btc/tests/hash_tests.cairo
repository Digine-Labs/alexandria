...
fn test_hash256_double_sha256() {
    ...
    assert(result != single_sha, 'assert failed');
}
...
fn test_sha256_from_byte_array() {
    ...
    assert(result == span_result, 'assert failed');
}
...
fn test_sha256_byte_array_output() {
    ...
    assert(result_array == array_result, 'assert failed');
}
...
fn test_different_inputs_produce_different_hashes() {
    ...
    assert(hash160_1 != hash160_2, 'assert failed');
}
...
fn test_large_data_hashing() {
    ...
    assert(sha256_result == sha256_result2, 'assert failed');
}
...
fn test_checksum_real_address_data() {
    ...
    assert(checksum_result == checksum_result2, 'assert failed');
}
...
fn test_hash256_vs_double_sha256() {
    ...
    assert(hash256_result == second_sha, 'assert failed');
    assert!(hash256_result.len() == 32);
}
...
fn test_empty_vs_single_byte() {
    ...
    assert(empty_sha != single_sha, 'assert failed');

    let empty_hash160 = hash160(empty_data);
    let single_hash160 = hash160(single_byte);

    assert(empty_hash160 != single_hash160, 'assert failed');
}
...
fn test_sha256_u256_range() {
    ...
    assert(result <= max_u256, 'assert failed');
}
...
fn test_hash_avalanche_effect() {
    ...
    assert(different_bytes > 10, 'assert failed');
}
...
fn test_all_ff_input() {
    ...
    assert(sha256_result != zero_sha256, 'assert failed');
    assert(hash160_result != zero_hash160, 'assert failed');
}
