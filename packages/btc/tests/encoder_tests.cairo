...
fn test_compact_size_encoding() {
    ...
    assert(encoder.data.at(0).unwrap() == 0x42, 'should be 0x42');
    assert(encoder.data.len() == 1, 'len should be 1');

    // Test 3-byte compact size (0xfd + 2 bytes)
    let mut encoder = TransactionEncoderTrait::new();
    encoder.write_compact_size(0x1234);
    assert(encoder.data.len() == 3, 'len should be 3'); // Total length should be 3
    assert(encoder.data.at(0).unwrap() == 0xfd, 'should be 0xfd');
    // Check little-endian encoding of 0x1234 (2 bytes)
    assert(encoder.data.at(1).unwrap() == 0x34, 'should be 0x34');
    assert(encoder.data.at(2).unwrap() == 0x12, 'should be 0x12');

    // Test 5-byte compact size (0xfe + 4 bytes)
    let mut encoder = TransactionEncoderTrait::new();
    encoder.write_compact_size(0x12345678);
    assert(encoder.data.len() == 5, 'len should be 5'); // Total length should be 5
    assert(encoder.data.at(0).unwrap() == 0xfe, 'should be 0xfe');
    // Check little-endian encoding of 0x12345678 (4 bytes)
    assert(encoder.data.at(1).unwrap() == 0x78, 'should be 0x78');
    assert(encoder.data.at(2).unwrap() == 0x56, 'should be 0x56');
    assert(encoder.data.at(3).unwrap() == 0x34, 'should be 0x34');
    assert(encoder.data.at(4).unwrap() == 0x12, 'should be 0x12');

    // Test 9-byte compact size (0xff + 8 bytes)
    let mut encoder = TransactionEncoderTrait::new();
    encoder.write_compact_size(0x1122334455667788);
    assert(encoder.data.len() == 9, 'len should be 9'); // Total length should be 9
    assert(encoder.data.at(0).unwrap() == 0xff, 'should be 0xff');
    // Check little-endian encoding of 0x1122334455667788 (8 bytes)
    assert(encoder.data.at(1).unwrap() == 0x88, 'should be 0x88');
    assert(encoder.data.at(2).unwrap() == 0x77, 'should be 0x77');
    assert(encoder.data.at(3).unwrap() == 0x66, 'should be 0x66');
    assert(encoder.data.at(4).unwrap() == 0x55, 'should be 0x55');
    assert(encoder.data.at(5).unwrap() == 0x44, 'should be 0x44');
    assert(encoder.data.at(6).unwrap() == 0x33, 'should be 0x33');
    assert(encoder.data.at(7).unwrap() == 0x22, 'should be 0x22');
    assert(encoder.data.at(8).unwrap() == 0x11, 'should be 0x11');
}
...
fn test_u32_le_encoding() {
    ...
    assert(encoder.data.at(0).unwrap() == 0x78, 'should be 0x78');
    assert(encoder.data.at(1).unwrap() == 0x56, 'should be 0x56');
    assert(encoder.data.at(2).unwrap() == 0x34, 'should be 0x34');
    assert(encoder.data.at(3).unwrap() == 0x12, 'should be 0x12');
}
...
fn test_u64_le_encoding() {
    ...
    // Verify little-endian encoding
    assert(encoder.data.at(0).unwrap() == 0x88, 'should be 0x88');
    assert(encoder.data.at(1).unwrap() == 0x77, 'should be 0x77');
    assert(encoder.data.at(2).unwrap() == 0x66, 'should be 0x66');
    assert(encoder.data.at(3).unwrap() == 0x55, 'should be 0x55');
    assert(encoder.data.at(4).unwrap() == 0x44, 'should be 0x44');
    assert(encoder.data.at(5).unwrap() == 0x33, 'should be 0x33');
    assert(encoder.data.at(6).unwrap() == 0x22, 'should be 0x22');
    assert(encoder.data.at(7).unwrap() == 0x11, 'should be 0x11');
}
...
fn test_simple_legacy_transaction_encoding() {
    ...
    assert(decoded_tx.version == 1, 'assert failed');
    assert(decoded_tx.inputs.len() == 1, 'inputs len should be 1');
    assert(decoded_tx.outputs.len() == 1, 'outputs len should be 1');
    assert(decoded_tx.locktime == 0, 'assert failed');
    assert(!decoded_tx.is_segwit, 'assert failed');

    // Check input
    let decoded_input = decoded_tx.inputs.at(0);
    assert(*decoded_input.previous_vout == 0, 'assert failed');
    assert(*decoded_input.sequence == 0xffffffff, 'assert failed');
    assert(decoded_input.script_sig.len() == 0, 'script_sig len should be 0');
    assert(decoded_input.previous_txid.len() == 32, 'txid len should be 32');

    // Check output
    let decoded_output = decoded_tx.outputs.at(0);
    assert(*decoded_output.value == 1, 'assert failed');
    assert(decoded_output.script_pubkey.len() == 25, 'script pubkey len 25');
}
...
fn test_segwit_transaction_encoding() {
    ...
    assert(encoded_data.at(4).unwrap() == 0x00, 'marker should be 0x00'); // Marker after version
    assert(encoded_data.at(5).unwrap() == 0x01, 'flag should be 0x01'); // Flag
    ...
    assert(decoded_tx.locktime == 0, 'assert failed');
    assert(decoded_tx.is_segwit, 'assert failed');
    assert(decoded_tx.witness.len() == 1, 'witness len should be 1');

    // Check witness data
    let decoded_witness = decoded_tx.witness.at(0);
    assert(decoded_witness.witness_stack.len() == 1, 'witness stack len 1');
    assert(decoded_witness.witness_stack.at(0).len() == 71, 'witness item len 71');
}
...
fn test_multi_input_output_transaction_encoding() {
    ...
    assert(decoded_tx.version == 1, 'assert failed');
    assert(decoded_tx.inputs.len() == 2, 'inputs len should be 2');
    assert(decoded_tx.outputs.len() == 2, 'outputs len should be 2');
    assert(decoded_tx.locktime == 0, 'assert failed');
    assert(!decoded_tx.is_segwit, 'assert failed');
    ...
    assert(*decoded_input1.previous_vout == 0, 'assert failed');
    assert(*decoded_input2.previous_vout == 1, 'assert failed');

    // Check outputs
    let decoded_output1 = decoded_tx.outputs.at(0);
    let decoded_output2 = decoded_tx.outputs.at(1);
    assert(*decoded_output1.value == 1000, 'assert failed');
    assert(*decoded_output2.value == 2000, 'assert failed');
}
...
fn test_round_trip_encoding_decoding() {
    ...
    assert(decoder.read_compact_size() == 252, 'should read 252');
    assert(decoder.read_compact_size() == 65535, 'should read 65535');
    assert(decoder.read_compact_size() == 4294967295, 'should read 4294967295');
}
...
fn test_bytes_encoding() {
    ...
    assert(encoder.data.len() == 4, 'data len should be 4');
    assert(encoder.data.at(0).unwrap() == 0x01, 'should be 0x01');
    assert(encoder.data.at(1).unwrap() == 0x02, 'should be 0x02');
    assert(encoder.data.at(2).unwrap() == 0x03, 'should be 0x03');
    assert(encoder.data.at(3).unwrap() == 0x04, 'should be 0x04');
}
