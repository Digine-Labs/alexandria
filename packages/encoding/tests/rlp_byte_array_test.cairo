...
fn test_ba_rlp_decode_type_short_list() {
    ...
    assert(result[1].len() == 0, 'assertion failed');
}
...
fn test_ba_rlp_decode_type_long_list() {
    ...
    assert(res[0] == @input_ba, 'assertion failed');
}
...
fn test_ba_rlp_decode_type_long_list_len_too_short() {
    ...
    assert(res.is_err(), 'assertion failed');
    assert(res.unwrap_err() == RLPError::InputTooShort, 'assertion failed');
}
...
fn test_ba_rlp_decode_type_long_string_payload_too_long() {
    ...
    assert(res.is_err(), 'assertion failed');
    assert(res.unwrap_err() == RLPError::PayloadTooLong, 'assertion failed');
}
...
fn test_ba_rlp_decode_empty() {
    ...
    assert(res.is_err(), 'assertion failed');
    assert(res.unwrap_err() == RLPError::EmptyInput, 'assertion failed');
}
...
fn test_ba_rlp_decode_long_string_with_payload_len_on_2_bytes() {
    ...
    assert(res.len() == 1, 'assertion failed');
    assert(res[0] == @expected_item, 'wrong value');
}
...
fn test_ba_rlp_encode_default_value() {
    ...
    assert(res.len() == 1, 'assertion failed');
    assert(res.at(0).unwrap() == 0x80, 'assertion failed');
}
...
fn test_ba_rlp_encode_string_single_byte_lt_0x80() {
    ...
    assert(res.len() == 1, 'assertion failed');
    assert(res.at(0).unwrap() == 0x40, 'assertion failed');
}
...
fn test_ba_rlp_encode_string_single_byte_ge_0x80() {
    ...
    assert(res.at(0).unwrap() == 0x81, 'assertion failed');
    assert(res.at(1).unwrap() == 0x80, 'assertion failed');
}
...
fn test_ba_rlp_encode_string_length_between_2_and_55() {
    ...
    assert(res.at(0).unwrap() == 0x82, 'assertion failed');
    assert(res.at(1).unwrap() == 0x40, 'assertion failed');
    assert(res.at(2).unwrap() == 0x50, 'assertion failed');
}
...
fn test_ba_rlp_encode_string_length_exactly_56() {
    ...
    let res = RLPTrait::encode_byte_array(array![input].span(), 0).unwrap();
    assert(res.len() == 58, 'assertion failed');
    assert(res.at(0).unwrap() == 0xB8, 'assertion failed');
    assert(res.at(1).unwrap() == 56, 'assertion failed');
    let (new_offset, result) = res.read_u256(0);
    assert(new_offset == 32, 'assertion failed');
    ...
    assert(result == 0x6060606060606060606060606060606060606060606060606060, 'assertion failed');
}
...
fn test_ba_rlp_encode_string_length_greater_than_56() {
    ...
    let input = RLPItemByteArray::String(input_ba);
    let res = RLPTrait::encode_byte_array(array![input].span(), 0).unwrap();
    assert(res.len() == 62, 'assertion failed');
    assert(res.at(0).unwrap() == 0xB8, 'assertion failed');
    assert(res.at(1).unwrap() == 60, 'assertion failed');
    let (new_offset, result) = res.read_u256(0);
    assert(new_offset == 32, 'assertion failed');
...
fn test_ba_rlp_encode_string_large_bytearray_inputs() {
    ...
    assert(res.len() == 503, 'assertion failed');
    assert(res.at(0).unwrap() == 0xb9, 'assertion failed');
    assert(res.at(1).unwrap() == 0x01, 'assertion failed');
    assert(res.at(2).unwrap() == 0xF4, 'assertion failed');
    let mut i = 3;
    while (i != 503) {
        assert(res.at(i).unwrap() == 0x70, 'assertion failed');
        i += 1;
    }
}
...
fn test_ba_rlp_encode_mutilple_string() {
    ...
    assert(res.len() == 10, 'assertion failed');
    assert(res.at(0).unwrap() == 0x88, 'assertion failed');
    assert(res.at(1).unwrap() == 0x40, 'assertion failed');
    assert(res.at(2).unwrap() == 0x53, 'assertion failed');
}
...
fn test_ba_rlp_encode_short_list() {
    ...
    assert(data == 0xc9833535354283453892, 'assertion failed');
}
...
fn test_ba_rlp_encode_short_nested_list() {
    ...
    assert(res.len() == 8, 'assertion failed');
    let (_, data) = res.read_uint_within_size(0, 8);
    assert(data == 0xc7c0c1c0c3c0c1c0, 'assertion failed');
}
...
fn test_rlp_encode_legacy_tx_calldata_long() {
    ...
    assert(res.len() == 517, 'assertion failed');
}
...
fn test_rlp_encode_legacy_tx_calldata_long_without_using_rlp_type() {
    ...
    assert(result.len() == 517, 'assertion failed');
}
...
fn test_ba_rlp_encode_short_nested_list_without_using_rlp_type() {
    ...
    assert(list.len() == 8, 'assertion failed');
