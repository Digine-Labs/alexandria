...
fn base64_encode_ba_simple_test() {
    ...
    assert(result.at(0).unwrap() == 'Y', 'assertion failed');
    assert(result.at(1).unwrap() == 'Q', 'assertion failed');
    assert(result.at(2).unwrap() == '=', 'assertion failed');
    assert(result.at(3).unwrap() == '=', 'assertion failed');
}
...
fn base64_encode_ba_hello_world_test() {
    ...
    assert(result == expected_ba, 'assertion failed');
}
...
fn base64_decode_ba_with_plus_and_slash() {
    ...
    assert(result.at(0).unwrap() == 255, 'assertion failed');
    assert(result.at(1).unwrap() == 239, 'assertion failed');
}
...
fn base64_ba_round_trip_test() {
    ...
    assert(decoded1.len() == ba.len(), 'length mismatch');
    assert(decoded1 == ba, 'assertion failed');
...
fn base64_ba_binary_data_test() {
    ...
    assert(decoded.len() == ba.len(), 'length mismatch');
    assert(decoded.at(0).unwrap() == 0, 'assertion failed');
    assert(decoded.at(1).unwrap() == 127, 'assertion failed');
    assert(decoded.at(2).unwrap() == 128, 'assertion failed');
    assert(decoded.at(3).unwrap() == 255, 'assertion failed');
}
...
fn base64_ba_longer_input_test() {
    ...
    i = 0;
    while i != ba.len() {
        assert(decoded.at(i) == ba.at(i), 'byte mismatch');
        i += 1;
    }
}
...
fn base64_ba_sample_byte_values_test() {
    ...
    while i != ba.len() {
        ...
        assert(decoded.at(0).unwrap() == val, 'assertion failed');
...
fn base64_ba_double_padding_test() {
    ...
    assert(encoded.at(2).unwrap() == '=', 'assertion failed');
    assert(encoded.at(3).unwrap() == '=', 'assertion failed');

    let decoded = Base64ByteArrayDecoder::decode(encoded);
    assert(decoded.len() == 1, 'assertion failed');
    assert(decoded == ba, 'assertion failed');
}
