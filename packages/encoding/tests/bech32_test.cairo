...
fn test_bech32_encode_decode() {
    ...
    // Check HRP matches
    assert(decoded_hrp == "bc", 'assert');

    // Check data matches
    assert(decoded_data.len() == data.len(), 'length mismatch');
    let mut i = 0;
    while i < data.len() {
        assert(*decoded_data.at(i) == *data.at(i), 'data mismatch');
        i += 1;
    };
}
...
fn test_convert_bits_no_padding() {
    ...
    assert(converted.len() > 0, 'conversion failed');
}
...
fn test_bech32_long_input() {
    ...
    assert(decoded_data.len() == long_data.len(), 'length mismatch');

    // Verify data content matches
    let mut j = 0;
    while j < long_data.len() {
        assert(*decoded_data.at(j) == *long_data.at(j), 'data mismatch');
        j += 1;
    };
}
...
fn test_bech32_address_length_limits() {
    ...
    assert(decoded_hrp == "bc", 'assert');
    assert(decoded_data.len() == max_data.len(), 'length mismatch');

    // Verify the encoded address respects BIP-173 90-character limit
    assert(encoded_max.clone().len() <= 90, 'exceeds max length');
}
