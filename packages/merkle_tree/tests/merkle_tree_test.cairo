...
fn test_odd_number_of_leaves() {
    ...
    assert(proof_5.len() > 0, '5 leaves valid proof');
    ...
    assert(is_valid_5, 'Proof 5 leaves valid');
    ...
    assert(proof_7.len() > 0, '7 leaves valid proof');
    ...
    assert(is_valid_7, 'Proof 7 leaves valid');
    ...
    assert(proof_9.len() > 0, '9 leaves valid proof');
    ...
    assert(is_valid_9, 'Proof 9 leaves valid');
}
...
fn test_single_leaf_edge_case() {
    ...
    assert(proof.len() == 1, 'Single leaf 1 proof');
    assert(*proof.at(0) == 0, 'Single leaf proof 0');
    ...
    assert(is_valid, 'Single leaf valid');
}
...
fn stored_merkle_tree_pedersen_test() {
    ...
    assert(computed_root == expected_root, 'assertion failed');

    // Get merkle proof.
    let computed_proof = StoredMerkleTreeImpl::<
        _, PedersenHasherImpl,
    >::get_proof(ref stored_tree, 0);
    assert(computed_proof == expected_proof, 'assertion failed');
    ...
    assert(!result, 'invalid proof fail');
...
fn stored_merkle_tree_odd_leaves_test() {
    ...
    assert(proof.len() > 0, 'Odd leaves valid proof');

    // Verify the proof works
    let result = StoredMerkleTreeImpl::<_, PoseidonHasherImpl>::verify(ref stored_tree, 3, proof);
    assert(result, 'Odd leaves proof valid');

    // [Assert] Get leaf count.
    let leaf_count = StoredMerkleTreeImpl::<_, PoseidonHasherImpl>::get_leaf_count(ref stored_tree);
    assert(leaf_count == 5, 'assertion failed');
}
...
fn stored_merkle_tree_single_leaf_test() {
    ...
    assert(result, 'Single leaf valid');

    // Get leaf count.
    let leaf_count = StoredMerkleTreeImpl::<_, PoseidonHasherImpl>::get_leaf_count(ref stored_tree);
    assert(leaf_count == 1, 'assertion failed');
}
...
fn stored_merkle_tree_empty_leaves_test() {
    ...
    assert(root == 0, 'assertion failed');

    // Get leaf count.
    let leaf_count = StoredMerkleTreeImpl::<_, PoseidonHasherImpl>::get_leaf_count(ref stored_tree);
    assert(leaf_count == 0, 'assertion failed');
}
...
fn stored_merkle_tree_efficiency_comparison_test() {
    ...
    for index in test_indices {
        ...
        assert(regular_proof == stored_proof, 'Proofs match');
        ...
        assert(regular_root == stored_root, 'Roots match');
        ...
        assert(regular_verify, 'Regular tree valid');
        assert(stored_verify, 'Stored tree valid');
    };
}
...
fn stored_merkle_tree_multi_proof_scenario_test() {
    ...
    for claimer in claimers {
        ...
        assert(is_valid, 'User valid airdrop');
        ...
        assert(computed_root == merkle_root, 'Proof verify root');
    }

    // Verify we generated exactly 4 proofs
    assert(all_proofs.len() == 4, 'Generated 4 proofs');
    ...
    assert(!is_invalid, 'Invalid user fail');
}
...
fn stored_merkle_tree_large_tree_test() {
    ...
    for index in test_indices {
        ...
        assert(is_valid, 'Large tree valid');
    };
}
...
fn stored_merkle_tree_out_of_bounds_test() {
    ...
    assert(!is_valid, 'Out of bounds invalid');
}
...
fn stored_merkle_tree_duplicate_leaves_test() {
    ...
    assert(valid_0, 'Duplicate 0 valid');
    assert(valid_1, 'Duplicate 1 valid');
    assert(valid_4, 'Duplicate 4 valid');
    ...
    assert(cross_valid, 'Same value diff pos');
}
...
fn stored_merkle_tree_zero_values_test() {
    ...
    assert(is_valid_zero, 'Zero value valid');
    ...
    assert(is_valid_one, 'Non-zero valid');
}
...
fn stored_merkle_tree_max_felt_values_test() {
    ...
    assert(is_valid, 'Large felt252 work');
}
