...
fn test_new() {
    ...
    assert(ba.len() == 48, 'ByteArray length mismatch');

    let (new_offset, result) = ba.read_u128(15);
    assert(new_offset == 31, 'value mismatch');
    assert(result == 0x16010203040506070809101112131415, 'value mismatch');
}

#[test]
fn test_new_empty() {
    let ba: ByteArray = ByteArrayTraitExt::new_empty();
    assert(ba.len() == 0, 'Empty ByteArray mismatch');
}
...
fn test_read_write_max_u16() {
    ...
    assert(ba.len() == 0, 'length should be 0');
    ba.append_u16(max_u16);
    let (new_offset, result) = ba.read_u16(0);
    assert(new_offset == 2, 'value mismatch');
    assert(result == max_u16, 'value mismatch');
}
...
fn test_read_write_max_u32() {
    ...
    assert(new_offset == 4, 'value mismatch');
    assert(result == max_u32, 'value mismatch');
}
...
fn test_read_zero_length_u128_array() {
    ...
    assert(new_offset == 0, 'value mismatch');
    assert(result.len() == 0, 'value mismatch');
}
...
fn test_append_u128() {
    ...
    assert(ba.len() == 16, 'value mismatch');
    assert(ba.at(0).unwrap() == 0x01, 'value mismatch');
    assert(ba.at(15).unwrap() == 0x16, 'value mismatch');
}
...
fn test_append_u256() {
    ...
    assert(ba.len() == 32, 'value mismatch');
    assert(ba.at(0).unwrap() == 0x01, 'value mismatch');
    assert(ba.at(31).unwrap() == 0x32, 'value mismatch');
}
...
fn test_append_felt252() {
    ...
    assert(ba.len() == 32, 'value mismatch');
    assert(ba.at(0).unwrap() == 0x00, 'value mismatch');
    assert(ba.at(31).unwrap() == 0x31, 'value mismatch');
}
...
fn test_read_felt252() {
    ...
    assert(result == felt_value, 'value mismatch');
}
...
fn test_read_u16_at_boundary() {
    ...
    assert(new_offset == 4, 'value mismatch');
    assert(result == 0x0506, 'value mismatch');
}
...
fn test_bytes_append() {
    ...
    assert(ba.len() == 2, 'value mismatch');
    assert(ba.at(0).unwrap() == 0x01, 'value mismatch');
    assert(ba.at(1).unwrap() == 0x02, 'value mismatch');

    ba.append_u32(0x03040506);
    assert(ba.len() == 6, 'value mismatch');
    assert(ba.at(2).unwrap() == 0x03, 'value mismatch');
    assert(ba.at(5).unwrap() == 0x06, 'value mismatch');

    ba.append_u64(0x0708091011121314);
    assert(ba.len() == 14, 'value mismatch');
    assert(ba.at(6).unwrap() == 0x07, 'value mismatch');
    assert(ba.at(13).unwrap() == 0x14, 'value mismatch');

    ba.append_usize(0x15161718);
    assert(ba.len() == 18, 'value mismatch');
    assert(ba.at(14).unwrap() == 0x15, 'value mismatch');
    assert(ba.at(17).unwrap() == 0x18, 'value mismatch');
    ...
    assert(ba2.len() == 32, 'value mismatch');
}
...
fn test_append_byte_31() {
    ...
    assert(val == b31, 'append_bytes31 mismatch');
}
...
fn test_read_u16() {
    ...
    assert(new_offset == 3, 'value mismatch');
    assert(result == 0x0405, 'value mismatch');
}
...
fn test_read_u32() {
    ...
    let (new_offset, result) = ba.read_u32(0);
    assert(new_offset == 4, 'value mismatch');
    assert(result == 0x03040506, 'value mismatch');
}
...
fn test_bytes_read_usize() {
    ...
    assert(result == 0x16010203, 'value mismatch');
}
...
fn test_read_u64() {
    ...
    assert(new_offset == 8, 'value mismatch');
    assert(result == 0x0708091011121314, 'value mismatch');
}
...
fn test_bytes_read_u256() {
    ...
    assert(result.high == 0x05060708091011121314151601020304, 'value mismatch');
    assert(result.low == 0x05060708091011121314151601020304, 'value mismatch');
}
...
fn test_read_bytes_size_0() {
    ...
    assert(new_offset == 0, 'value mismatch');
    assert(result == Default::default(), 'result mismatch');
}
...
fn test_read_bytes_lower_than_32() {
    ...
    assert(new_offset == 3, 'value mismatch');
    assert(result == expected_result, 'value mismatch');
}
...
fn test_read_bytes_greater_than_32() {
    ...
    assert(new_offset == 7, 'value mismatch');
    assert(result == expected_result, 'value mismatch');
}
...
fn test_update_at() {
    ...
    ba.update_at(1, 0x07);
    assert(ba.len() == expected_result.len(), 'length mismatch');
    assert(ba == expected_result, 'value mismatch');
}
...
fn test_bytes_read_bytes31() {
    ...
    assert(val == byte31, 'read_bytes31 mismatch');
}
...
fn test_span_u8_into_byte_array() {
    ...
    while let Option::Some(byte) = ba.at(index) {
        assert(*(array[index]) == byte, 'array byte mismatch');
        index += 1;
    };
}
...
fn test_keccak_be() {
    ...
    assert(keccak_be_result == solidity_expected, 'value mismatch');
}
...
fn test_keccak_le() {
    ...
    assert(keccak_le_result == expected, 'value mismatch');
}
...
fn test_keccak_le_be_endian_difference() {
    ...
    assert(keccak_le_result != keccak_be_result, 'keccak results should differ');
    ...
    assert(keccak_le_reversed == keccak_be_result, 'value mismatch');
    ...
    assert(keccak_be_reversed == keccak_le_result, 'value mismatch');
}
