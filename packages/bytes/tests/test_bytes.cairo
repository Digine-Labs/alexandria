...
fn test_bytes_zero() {
    ...
    assert(bytes.size() == 1, 'size mismatch');
    assert(*bytes[0] == 0, 'value mismatch');

    let bytes = BytesTrait::zero(17);
    assert(bytes.size() == 17, 'size mismatch');
    assert(*bytes[0] == 0, 'value mismatch');
    assert(*bytes[1] == 0, 'value mismatch');
    let (_, value) = bytes.read_u8(15);
    assert(value == 0, 'value mismatch');
...
fn test_bytes_update() {
    ...
    assert(bytes.size() == 5, 'size mismatch');
    assert(*bytes[0] == 0x05020304050000000000000000000000, 'value mismatch');

    bytes.update_at(1, 0x06);
    assert(bytes.size() == 5, 'size mismatch');
    assert(*bytes[0] == 0x05060304050000000000000000000000, 'value mismatch');

    bytes.update_at(2, 0x07);
    assert(bytes.size() == 5, 'size mismatch');
    assert(*bytes[0] == 0x05060704050000000000000000000000, 'value mismatch');

    bytes.update_at(3, 0x08);
    assert(bytes.size() == 5, 'size mismatch');
    assert(*bytes[0] == 0x05060708050000000000000000000000, 'value mismatch');

    bytes.update_at(4, 0x09);
    assert(bytes.size() == 5, 'size mismatch');
    assert(*bytes[0] == 0x05060708090000000000000000000000, 'value mismatch');
    ...
    assert(bytes.size() == 42, 'size mismatch');
    assert(*bytes[0] == 0x01020304050607080910111213141516, 'value mismatch');
    assert(*bytes[1] == 0x16020304050607080910111213141516, 'value mismatch');
    assert(*bytes[2] == 0x01020304050607080910000000000000, 'value mismatch');

    bytes.update_at(15, 0x01);
    assert(bytes.size() == 42, 'size mismatch');
    assert(*bytes[0] == 0x01020304050607080910111213141501, 'value mismatch');
    assert(*bytes[1] == 0x16020304050607080910111213141516, 'value mismatch');
    assert(*bytes[2] == 0x01020304050607080910000000000000, 'value mismatch');
}
...
fn test_bytes_read_u128_packed() {
    ...
    assert(new_offset == 1, 'value mismatch');
    assert(value == 0x01, 'value mismatch');

    let (new_offset, value) = bytes.read_u128_packed(new_offset, 14);
    assert(new_offset == 15, 'value mismatch');
    assert(value == 0x0203040506070809101112131415, 'value mismatch');

    let (new_offset, value) = bytes.read_u128_packed(new_offset, 15);
    assert(new_offset == 30, 'value mismatch');
    assert(value == 0x160102030405060708091011121314, 'value mismatch');

    let (new_offset, value) = bytes.read_u128_packed(new_offset, 8);
    assert(new_offset == 38, 'value mismatch');
    assert(value == 0x1516010203040506, 'value mismatch');

    let (new_offset, value) = bytes.read_u128_packed(new_offset, 4);
    assert(new_offset == 42, 'value mismatch');
    assert(value == 0x07080910, 'value mismatch');
}
...
fn test_bytes_read_u128_array_packed() {
    ...
    assert(new_offset == 9, 'value mismatch');
    assert(*new_array[0] == 0x010203, 'value mismatch');
    assert(*new_array[1] == 0x040506, 'value mismatch');
    assert(*new_array[2] == 0x070809, 'value mismatch');

    let (new_offset, new_array) = bytes.read_u128_array_packed(9, 4, 7);
    assert(new_offset == 37, 'value mismatch');
    assert(*new_array[0] == 0x10111213141516, 'value mismatch');
    assert(*new_array[1] == 0x01020304050607, 'value mismatch');
    assert(*new_array[2] == 0x08091011121314, 'value mismatch');
    assert(*new_array[3] == 0x15160102030405, 'value mismatch');
}
...
fn test_bytes_read_felt252_packed() {
    ...
    assert(new_offset == 33, 'value mismatch');
    assert(value == 0x1415160102030405060708091011121314151601, 'value mismatch');
}
...
fn test_bytes_read_u8() {
    ...
    assert(new_offset == 16, 'value mismatch');
    assert(value == 0x16, 'value mismatch');
}
...
fn test_bytes_read_u16() {
    ...
    assert(new_offset == 17, 'value mismatch');
    assert(value == 0x1601, 'value mismatch');
}
...
fn test_bytes_read_u32() {
    ...
    assert(new_offset == 19, 'value mismatch');
    assert(value == 0x16010203, 'value mismatch');
}
...
fn test_bytes_read_u64() {
    ...
    assert(new_offset == 23, 'value mismatch');
    assert(value == 0x1601020304050607, 'value mismatch');
}
...
fn test_bytes_read_u128() {
    ...
    assert(new_offset == 31, 'value mismatch');
    assert(value == 0x16010203040506070809101112131415, 'value mismatch');
}
...
fn test_bytes_read_u256() {
    ...
    assert(new_offset == 36, 'value mismatch');
    assert(value.high == 0x05060708091011121314151601020304, 'value mismatch');
    assert(value.low == 0x05060708091011121314151601020304, 'value mismatch');
}
...
fn test_bytes_read_bytes31() {
    ...
    assert(offset == 33, 'value mismatch');
...
fn test_bytes_read_u256_array() {
    ...
    assert(new_offset == 71, 'value mismatch');
    let result: u256 = *new_array[0];
    assert(result.high == 0x08091011121314151616151413121110, 'value mismatch');
    assert(result.low == 0x09080706050403020116151413121110, 'value mismatch');
    let result: u256 = *new_array[1];
    assert(result.high == 0x09080706050403020101020304050607, 'value mismatch');
    assert(result.low == 0x08091011121314151601020304050607, 'value mismatch');
}
...
fn test_bytes_read_address() {
    ...
    assert(new_offset == 46, 'value mismatch');
    assert(value.into() == address, 'value mismatch');
}
...
fn test_bytes_read_bytes() {
    ...
    assert(new_offset == 41, 'value mismatch');
    assert(sub_bytes.size() == 37, 'size mismatch');
    assert(*sub_bytes_data[0] == 0x05060708091011121314015401855d77, 'value mismatch');
    assert(*sub_bytes_data[1] == 0x96176b05d160196ff92381eb7910f544, 'value mismatch');
    assert(*sub_bytes_data[2] == 0x6c2e0e04e10000000000000000000000, 'value mismatch');

    let (new_offset, sub_bytes) = bytes.read_bytes(0, 14);
    let sub_bytes_data = @sub_bytes;
    assert(new_offset == 14, 'value mismatch');
    assert(sub_bytes.size() == 14, 'size mismatch');
    assert(*sub_bytes_data[0] == 0x01020304050607080910111213140000, 'value mismatch');

    // read first byte
    let (new_offset, sub_bytes) = bytes.read_bytes(0, 1);
    let sub_bytes_data = @sub_bytes;
    assert(new_offset == 1, 'value mismatch');
    assert(sub_bytes.size() == 1, 'size mismatch');
    assert(*sub_bytes_data[0] == 0x01000000000000000000000000000000, 'value mismatch');
    ...
    assert(new_offset == 46, 'value mismatch');
    assert(sub_bytes.size() == 1, 'size mismatch');
    assert(*sub_bytes_data[0] == 0x4f000000000000000000000000000000, 'value mismatch');
}
...
fn test_bytes_append() {
    ...
    assert(bytes.size() == 9, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171800000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());

    bytes.append_u128_packed(0x101112131415161718, 9);
    assert(bytes.size() == 18, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180000000000000000000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());

    // append_u8
    bytes.append_u8(0x01);
    assert(bytes.size() == 19, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180100000000000000000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());

    // append_u16
    bytes.append_u16(0x0102);
    assert(bytes.size() == 21, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180101020000000000000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());

    // append_u32
    bytes.append_u32(0x01020304);
    assert(bytes.size() == 25, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180101020102030400000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());

    // append_usize
    bytes.append_usize(0x01);
    assert(bytes.size() == 29, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180101020102030400000001000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());

    // append_u64
    bytes.append_u64(0x030405060708);
    assert(bytes.size() == 37, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180101020102030400000001000003, 'value mismatch');
    assert(*bytes[2] == 0x04050607080000000000000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());

    // append_u128
    bytes.append_u128(0x101112131415161718);
    assert(bytes.size() == 53, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180101020102030400000001000003, 'value mismatch');
    assert(*bytes[2] == 0x04050607080000000000000010111213, 'value mismatch');
    assert(*bytes[3] == 0x14151617180000000000000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());

    // append_u256
    bytes.append_u256(u256 { low: 0x01020304050607, high: 0x010203040506070809 });
    assert(bytes.size() == 85, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180101020102030400000001000003, 'value mismatch');
    assert(*bytes[2] == 0x04050607080000000000000010111213, 'value mismatch');
    assert(*bytes[3] == 0x14151617180000000000000001020304, 'value mismatch');
    assert(*bytes[4] == 0x05060708090000000000000000000102, 'value mismatch');
    assert(*bytes[5] == 0x03040506070000000000000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());
    ...
    assert(bytes.size() == 117, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180101020102030400000001000003, 'value mismatch');
    assert(*bytes[2] == 0x04050607080000000000000010111213, 'value mismatch');
    assert(*bytes[3] == 0x14151617180000000000000001020304, 'value mismatch');
    assert(*bytes[4] == 0x05060708090000000000000000000102, 'value mismatch');
    assert(*bytes[5] == 0x0304050607015401855d7796176b05d1, 'value mismatch');
    assert(*bytes[6] == 0x60196ff92381eb7910f5446c2e0e04e1, 'value mismatch');
    assert(*bytes[7] == 0x3db2194a4f0000000000000000000000, 'value mismatch');
}
...
fn test_bytes_concat() {
    ...
    bytes.concat(@other);
    assert(bytes.size() == 163, 'size mismatch');
    assert(*bytes[0] == 0x10111213141516171810111213141516, 'value mismatch');
    assert(*bytes[1] == 0x17180101020102030400000001000003, 'value mismatch');
    assert(*bytes[2] == 0x04050607080000000000000010111213, 'value mismatch');
    assert(*bytes[3] == 0x14151617180000000000000001020304, 'value mismatch');
    ...
    assert(*bytes[5] == 0x0304050607015401855d7796176b05d1, 'value mismatch');
    assert(*bytes[6] == 0x60196ff92381eb7910f5446c2e0e04e1, 'value mismatch');
    assert(*bytes[7] == 0x3db2194a4f0102030405060708091011, 'value mismatch');
    assert(*bytes[8] == 0x121314015401855d7796176b05d16019, 'value mismatch');
    assert(*bytes[9] == 0x6ff92381eb7910f5446c2e0e04e13db2, 'value mismatch');
    assert(*bytes[10] == 0x194a4f00000000000000000000000000, 'value mismatch');
    bytes = BytesTrait::new(bytes.size(), bytes.data());
...
fn test_bytes_keccak() {
    ...
    assert(res == hash, 'value mismatch');
...
fn test_byte_array_conversions() {
    ...
    assert(bytes == new_bytes, 'value mismatch');
}
...
fn test_bytes_new_with_padded_first() {
    ...
    assert(new_offset == 16, 'value mismatch');
    assert(value == 0x00000000000000000000000000000123, 'value mismatch');
}
...
fn test_bytes_new_with_padded_mid() {
    ...
    assert(new_offset == 32, 'value mismatch');
    assert(value == 0x00000000000000000000000000000123, 'value mismatch');
}
...
fn test_bytes_new_with_padded_last() {
    ...
    assert(new_offset == 48, 'value mismatch');
    assert(value == 0x00000000000000000000000000000123, 'value mismatch');
}
...
fn test_bytes_new_with_padded_multi() {
    ...
    let (new_offset, value) = bytes.read_u128_packed(32, 16);
    assert(new_offset == 48, 'value mismatch');
    assert(value == 0x01020304050607080910111213141516, 'value mismatch');

    let (new_offset, value) = bytes.read_u128(48);
    assert(new_offset == 64, 'value mismatch');
    assert(value == 0x00000000000000000000000000001234, 'value mismatch');

    let (new_offset, value) = bytes.read_u128_packed(64, 16);
    assert(new_offset == 80, 'value mismatch');
    assert(value == 0x00000000000000000000000000123450, 'value mismatch');
}
